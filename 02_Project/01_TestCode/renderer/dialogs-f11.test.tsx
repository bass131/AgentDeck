...
import type { ConversationRecord } from '../../../02_Project/00_Source/shared/ipcContract'
...
describe('F11-02: PromptModal', () => {
  async function renderPromptModal(overrides: Partial<{
    ...
    const { PromptModal } = await import('../../../02_Project/00_Source/renderer/src/features/prompt')
    const props = {
      target: '채팅 1',
      scope: '이 채팅에만 적용',
      noun: '채팅',
...
describe('F11-02: FolderSwitchDialog', () => {
  async function renderFolderSwitch(overrides: Partial<{
    ...
    const { FolderSwitchDialog } = await import('../../../02_Project/00_Source/renderer/src/features/file')
    const props = {
...
describe('F11-02: Sidebar ctx-menu 프롬프트 설정 → PromptModal', () => {
  ...
  async function renderSidebar() {
    const { Sidebar } = await import('../../../02_Project/00_Source/renderer/src/features/shell/Sidebar')
...
describe('F11-03: AskModal', () => {
  async function renderAskModal(overrides: Partial<{
    ...
    const { AskModal } = await import('../../../02_Project/00_Source/renderer/src/features/prompt')
    const props = {
      minimized: false,
      onClose: vi.fn(),
...
describe('F11-03: Composer onSlashAsk prop (하위호환 + 신규)', () => {
  function mkProps(over: Partial<Parameters<typeof import('../../../02_Project/00_Source/renderer/src/features/conversation/Composer').Composer>[0]> = {}) {
    return {
      value: '',
      onChange: vi.fn(),
      onSend: vi.fn(),
