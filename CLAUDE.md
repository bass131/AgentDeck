...
- **무인 배치(영호 부재)는 원칙 금지** — 본 모드는 *attended 루프*(영호 감독 하 자율 진행)다. **유일 예외 = 야간 자율 런(night-run, 영호 승인 2026-07-28)**: 영호가 자기 전 *명시 기동*한 런에 한해 무인 지속을 허용 — 버킷 (a)만·로컬 커밋까지·질문 대신 스킵. 예약·cron 무인 기동은 여전히 금지. 상세 = `loop-driver.md` §3-1.
