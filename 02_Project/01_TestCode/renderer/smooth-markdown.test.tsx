...
describe('smoothRevealStep — 순수 함수 velocity 공식', () => {
  it('exports smoothRevealStep 함수', async () => {
    const mod = await import('../../../02_Project/00_Source/renderer/src/lib/smoothReveal')
    expect(typeof mod.smoothRevealStep).toBe('function')
  })

  it('buffer > 0일 때 cur가 증가한다(단조 증가)', async () => {
    const { smoothRevealStep } = await import('../../../02_Project/00_Source/renderer/src/lib/smoothReveal')
...
describe('SmoothMarkdown 컴포넌트', () => {
  ...
  it('exports SmoothMarkdown (memo → function 또는 object)', async () => {
    const mod = await import('../../../02_Project/00_Source/renderer/src/features/conversation/SmoothMarkdown')
