...
describe('resolveLoopStatus — goal-stale 변형 (BL1 P03)', () => {
  it('autonomyActive=true + bannerStale=true → goal-stale', () => {
    const st = resolveLoopStatus([], { name: 'goal', turns: 3, detail: '문서 정리' }, false, true, true, false)
    expect(st.kind).toBe('goal-stale')
  })

  it('autonomyActive=true + bannerStale=false → 기존과 동일하게 goal(회귀 0)', () => {
    const st = resolveLoopStatus([], { name: 'goal', turns: 3 }, false, true, false, false)
    expect(st.kind).toBe('goal')
  })

  it('bannerStale=true여도 staleDismissed=true면 표시가 숨겨진다(none) — 수동 해제', () => {
    const st = resolveLoopStatus([], { name: 'goal', turns: 3 }, false, true, true, true)
    expect(st.kind).toBe('none')
  })

  it('staleDismissed=true + stoppedNotice=true → stopped(정지확인이 있으면 그쪽을 보여준다)', () => {
    const st = resolveLoopStatus([], null, true, true, true, true)
  ...
  it('단일 표시 불변식: sdk가 goal-stale보다 우선', () => {
    const st = resolveLoopStatus(
      ...
      { name: 'goal', turns: 1 }, false, true, true, false,
    )
    expect(st.kind).toBe('sdk')
  })

  it('autonomyActive=false면 bannerStale=true여도 goal-stale이 아니다(게이트 우선)', () => {
    const st = resolveLoopStatus([], { name: 'goal', turns: 1 }, false, false, true, false)
    expect(st.kind).toBe('none')
  })

  it('4번째~6번째 인자 전부 미전달(기존 3-4인자 호출부) → 회귀 0', () => {
    expect(resolveLoopStatus([], { name: 'goal', turns: 2 }, undefined, true).kind).toBe('goal')
  })
})
...
describe('appStore(단일챗) — foreground stale-watchdog 라이브 배선 (fake timer)', () => {
  ...
  it('refreshStaleWatchdog: 신선한 lastActivityAt → 남은 시간 경과 후 자동으로 bannerStale=true(setTimeout 1회, 폴링 없음)', () => {
    const t0 = Date.now()
    useAppStore.setState({ autonomyActive: true, lastActivityAt: t0, bannerStale: false } as Parameters<typeof useAppStore.setState>[0])
  ...
  it('refreshStaleWatchdog: autonomyActive=false → 타이머 미가동(경과해도 bannerStale 불변)', () => {
    useAppStore.setState({ autonomyActive: false, lastActivityAt: Date.now(), bannerStale: false } as Parameters<typeof useAppStore.setState>[0])
    useAppStore.getState().refreshStaleWatchdog()
    vi.advanceTimersByTime(GOAL_BANNER_STALE_THRESHOLD_MS + 1000)
    expect(useAppStore.getState().bannerStale).toBe(false)
  })
  ...
  it('실시간 활동 신호(subscribeAgentEvents 경로1) 도착 시 타이머가 재무장되고 bannerStale이 해제된다', () => {
    useAppStore.setState({ currentRunId: 'run-live', autonomyActive: true, lastActivityAt: Date.now(), bannerStale: false } as Parameters<typeof useAppStore.setState>[0])
  ...
  it('정상 경로 회귀: abortRun은 bannerStale/staleDismissed/lastActivityAt을 리셋한다', async () => {
    useAppStore.setState({
  ...
  it('정상 경로 회귀: ended(autonomy_status) 도착 시 기존 해제 동작 불변 + stale 필드도 함께 정리', () => {
    useAppStore.setState({ currentRunId: 'run-ended', autonomyActive: true, bannerStale: true, staleDismissed: true } as Parameters<typeof useAppStore.setState>[0])
    const unsubscribe = useAppStore.getState().subscribeAgentEvents()
...
describe('대화 전환 연속성 — bgRuns 복귀 (BG_RUNS_CAP 이내)', () => {
  ...
  it('A(goal 진행, 아직 신선) → B로 전환 → 임계 초과만큼 대기 → A로 복귀 시 즉시 stale로 표시된다(타이머가 리셋되지 않는다)', async () => {
    const t0 = Date.now()
    useAppStore.setState({
      conversationId: 'A', currentRunId: 'run-a', autonomyActive: true, lastActivityAt: t0, bannerStale: false,
    } as Parameters<typeof useAppStore.setState>[0])
