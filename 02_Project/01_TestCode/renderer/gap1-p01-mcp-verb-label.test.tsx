...
describe('mcpToolLabel — mcp__server__tool → "서버 · 도구" 파싱 (순수)', () => {
  it('mcp__claude_ai_Notion__notion-search → "claude_ai_Notion · notion-search"', async () => {
    const { mcpToolLabel } = await import('../../../02_Project/00_Source/renderer/src/lib/toolKind')
...
describe('toolMetaFor — mcp 분기가 mcpToolLabel을 verb로 사용', () => {
  it('원시 mcp__ 도구명 → verb가 "서버 · 도구" 라벨', async () => {
    const { toolMetaFor } = await import('../../../02_Project/00_Source/renderer/src/lib/toolKind')
...
describe('ToolCallCard — 접힘 한 줄(.t-verb)에 MCP 라벨 노출', () => {
  it('mcp__ 도구 카드 → .t-verb가 raw 전체 이름이 아니라 "서버 · 도구"', async () => {
    const { ToolCallCard } = await import('../../../02_Project/00_Source/renderer/src/features/conversation/ToolCallCard')
    const card = {
      id: 'mcp1',
...
describe('SubAgentInline — 실행 중 도구 활동 행에 MCP 라벨 노출', () => {
  it('runningTool.verb가 raw mcp 이름이어도 활동 행은 "서버 · 도구" 표시', async () => {
    const { SubAgentInline } = await import('../../../02_Project/00_Source/renderer/src/features/agent/SubAgentInline')
    const agent = {
      id: 'sa1',
      name: 'explorer',
      role: '탐색',
...
describe('SubAgentModal — 펼침 도구 행(.sa-tool-verb)에 MCP 라벨 노출', () => {
  it('t.verb가 raw mcp 이름이어도 .sa-tool-verb는 "서버 · 도구" 표시', async () => {
    const { SubAgentModal } = await import('../../../02_Project/00_Source/renderer/src/features/agent/SubAgentModal')
    const agent = {
      id: 'sa1',
      name: 'explorer',
      role: '탐색',
