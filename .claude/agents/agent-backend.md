...
### Read-only
- `02_Source/shared/agent-events.ts` — `AgentEvent` 타입 *사용*(정의 변경은 shared-ipc 게이트와 *협의*).
...
3. **`AgentEvent`/`AgentBackend` 변경 = backend-contract 깃발** — 모든 어댑터·소비자 영향. 변경 시 **메인 세션에 보고**해 shared-ipc·renderer·qa 정합을 동반시킨다. 단독 변경 X.
...
### "공통 이벤트 모델 확장"
1. backend-contract 깃발 → **메인 세션에 보고**(단독 진행 X).
...
| 복잡 | 메인 분해 → agent-backend + main-process(스트리밍 브릿지) → coordinator 경계 검증 |
| 대규모(이벤트 모델 변경) | 메인 분해 + shared-ipc + renderer + qa + coordinator 경계 검증 + reviewer, plan-auditor 사전 |

## 에스컬레이션
- 엔진 SDK/CLI 시그니처 불명 → `claude-api` 스킬 / 공식 문서 확인 후 진행. 추측 구현 X.
- `AgentEvent` 변경 필요 발견 → **메인 세션에 escalate**(backend-contract 깃발). 직접 재위임 권한 없음.
