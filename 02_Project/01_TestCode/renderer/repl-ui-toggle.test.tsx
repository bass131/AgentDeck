...
import { makeInitialState, applyAgentEvent } from '../../../02_Project/00_Source/renderer/src/store/reducer'
...
describe('RU-A: REPL 토글 버튼 store 계약', () => {
  ...
  it('A-2: selectReplMode 셀렉터가 replMode를 반환', async () => {
    const { selectReplMode } = await import('../../../02_Project/00_Source/renderer/src/store/appStore')
  ...
  it('A-4: setReplMode(!current) 토글 — true→false→true', async () => {
    const cur1 = useAppStore.getState().replMode
    useAppStore.getState().setReplMode(!cur1)
    const cur2 = useAppStore.getState().replMode
    expect(cur2).toBe(false)
    useAppStore.getState().setReplMode(!cur2)
    expect(useAppStore.getState().replMode).toBe(true)
  })
})
