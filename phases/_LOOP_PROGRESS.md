...
## ⏹ 루프 정지 — 자율 범위 소진 (사용자 결정 2건 대기)
**무인으로 가능한 작업 완료**: M4-4 ✅ + B9 ✅ (전부 커밋·reviewer 🔴 0·라이브 검증). 남은 B8·M2-LSP는 **둘 다 사용자 결정 게이트** → 루프 정책("ADR/정책 변경=사용자 단독→보고 후 정지")대로 정지.

### 결정 ① — B8(사용량 분석)
원본에 비용/히스토리 시스템 **없음**. 실제 "사용량"=`~/.claude/.credentials.json` OAuth 토큰 읽어 비공개 베타 엔드포인트(`api/oauth/usage`)로 레이트리밋 게이지(5h·주간). **옵션**: ⓐ 1:1 복제(자격증명+미문서 API, main 단독) / ⓑ 스킵(컨텍스트 게이지로 충분) / ⓒ 신규 비용시스템(원본 밖=비권장).

### 결정 ② — M2-LSP(C5 호버/정의이동·C2 시맨틱)
원본은 LSP **완전 구현**(`src/main/lsp/` ~1500줄, `typescript-language-server`+`pyright` 의존성 ~65MB, JSON-RPC, 신규 IPC 15채널). 복제 시 **ADR-017 신설 필요**(의존성·번들크기·다운로드형[C#/C++] 범위 트레이드오프). **헌법: 의존성/ADR=사용자 단독 통제** → 무인 불가. 사용자가 ADR-017 승인 시 진행(서브웨이브: ⓐ 번들서버[TS/Py] → ⓑ 렌더 통합[CmEditor 호버/F12/색칠] → ⓒ 다운로드형).
