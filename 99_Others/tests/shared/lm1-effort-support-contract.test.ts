 ...
 *
 * TDD 순서: 이 파일이 먼저 작성(실패) → shared-ipc가 02_Source/shared/modelEffort.ts를
 * 생성(표 + EffortSupport 값 타입 export) → agent-backend가 runArgs.ts에서 표 정의를
 * 제거하고 shared에서 import + re-export → 이 파일이 GREEN.
 *
 * 대상(R only — 구현은 shared-ipc·agent-backend Worker 몫):
 *   02_Source/shared/modelEffort.ts (신규) — `MODEL_EFFORT_SUPPORT` 상수 +
 *     값 타입 `EffortSupport`({ supports:boolean; xhigh?:boolean }). 순수 도메인 상수 모듈
 *     (IPC 채널 아님 · Node 전용 API 금지 · main·renderer 양쪽 import 대상).
 *   02_Source/main/01_agents/runArgs.ts — 기존 :54 정의를 제거하고 shared에서
 ...
 *      keys(MODEL_CONTEXT_WINDOW). runArgs.ts:25·:146의 "드리프트 금지" 주석 계약을
 ...
 *
 * import 경로 관례:
 *   - MODEL_CONTEXT_WINDOW: 배럴 02_Source/shared/ipcContract 경유(ipcContract.ts:51의
...
import {
  ...
  KNOWN_MODELS,
} from '../../../02_Source/main/01_agents/runArgs'
...
describe('LM1 P06 — MODEL_EFFORT_SUPPORT shared 형상 (RED)', () => {
  it('각 값이 { supports:boolean, xhigh?:boolean } 형상이다 (구조적 typeof)', () => {
    // RED: shared/modelEffort 모듈 미존재 → import 해석 실패로 이 스위트가 로드되지 않는다.
