...
| `main` | `src/main/**` (단 `src/main/ipc/**` = ⛔ G7) | ✅ + 🔶 | main-process |
| `backend` | `src/main/agents/**` (ADR-003 엔진 경계 신중 — 어댑터 내부 리터럴 이동 X) | ✅ + 🔶 | agent-backend |
| `renderer` | `src/renderer/**` 로직(store/hooks/util) | ✅ + 🔶 / **시각·CSS·JSX 레이아웃 = 📋 제안만(G3)** | renderer |
| `qa` | `tests/**` (테스트 구조 정리·중복 제거) | ✅ + 🔶 | qa |
| **항상 제외** | `src/preload/**`·`src/main/ipc/**`·`canUseTool`/권한 경로·`*.config.*`·`*.d.ts`·`.env*`·생성물 | ⛔ | — |
...
결과를 합쳐 **개선 백로그** 생성 + 아래 위험도표로 `✅/🔶/⛔/📋` 라벨 + 우선순위. (백로그 시드 = `docs/HARNESS_GAP.md` §거대파일·중복.)
...
> - **📋 제안만 (검증 불가)** — renderer 시각/CSS/레이아웃(UI_GUIDE 육안). 진단·제안 diff까지.
...
| **위치: `src/preload/**`·`src/main/ipc/**`·`canUseTool`/권한 경로** | ⛔ 영구 제외 (신뢰경계 G7) |
| **ADR-003: 엔진 고유 리터럴(resume/Cron/Workflow/SDKUserMessage)을 어댑터 밖으로 이동** | ⛔ 영구 제외 (G7) |
| **위치: renderer 시각(.css·JSX 레이아웃·애니메이션)** | 📋 제안만 (UI_GUIDE 육안, G3) |
...
3. **renderer 시각 제안만 (G3)** — UI_GUIDE 육안 검증 불가. commit X, 제안 diff까지.
...
7. **신뢰경계 + ADR-003 영구 제외 (G7)** — `src/preload`·`src/main/ipc`·`canUseTool`·엔진 리터럴 이동 = 영원히 사람 트랙.
