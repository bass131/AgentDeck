...
describe('F11-02: PromptModal', () => {
  async function renderPromptModal(overrides: Partial<{
    ...
    const { PromptModal } = await import('../../../02.Source/renderer/src/components/06_prompt/PromptModal')
    const props = {
      target: '채팅 1',
      scope: '이 채팅에만 적용',
      noun: '채팅',
...
describe('F11-02: FolderSwitchDialog', () => {
  async function renderFolderSwitch(overrides: Partial<{
    ...
    const { FolderSwitchDialog } = await import('../../../02.Source/renderer/src/components/02_file/FolderSwitchDialog')
    const props = {
...
describe('F11-03: AskModal', () => {
  async function renderAskModal(overrides: Partial<{
    ...
    const { AskModal } = await import('../../../02.Source/renderer/src/components/06_prompt/AskModal')
    const props = {
      minimized: false,
      onClose: vi.fn(),
...
describe('F11-03: Composer onSlashAsk prop (하위호환 + 신규)', () => {
  function mkProps(over: Partial<Parameters<typeof import('../../../02.Source/renderer/src/components/01_conversation/Composer').Composer>[0]> = {}) {
    return {
      value: '',
      onChange: vi.fn(),
      onSend: vi.fn(),
