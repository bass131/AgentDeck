...
describe('W6a — CM6 search 확장', () => {
  it('buildBaseExtensions 호출 시 search() 확장이 포함된다', async () => {
    const { CodeViewer } = await import('../../../02_Project/00_Source/renderer/src/features/viewer')
    await act(async () => {
...
describe('W6b — SelectionAskBar', () => {
  it('CodeViewerProps에 onAskSelection prop이 있다', async () => {
    const mod = await import('../../../02_Project/00_Source/renderer/src/features/viewer')
  ...
  it('선택 텍스트가 있을 때 sel-bar가 표시된다', async () => {
    const { SelectionAskBar } = await import('../../../02_Project/00_Source/renderer/src/features/viewer')
    const onAsk = vi.fn()
    let container!: HTMLElement
    await act(async () => {
      const result = render(
...
describe('W6b — injectedInput 형식', () => {
  it('buildAskPayload이 올바른 형식의 텍스트를 생성한다', async () => {
    const { buildAskPayload } = await import('../../../02_Project/00_Source/renderer/src/features/viewer')
