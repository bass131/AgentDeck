...
// 케이스는 grace 만료까지 시간이 필요하다. 가짜 타이머로 결정적으로 제어한다.
// **현재(즉시 close) 코드에서도 무해**: 타이머가 없으면 advanceTimersByTimeAsync는 microtask
// flush + no-op이고, 즉시 close 경로는 그대로 동작한다. 계약 의미(무활동 자연종료 → 후속
// resume 회상)는 보존 — 소비 promise를 먼저 시작 → grace 만료 advance → 소비 완료 await
// 패턴으로 **타이머 진행만 삽입**하고 각 케이스의 최종 assert는 불변으로 둔다.
const GRACE_EXPIRE_MS = 10_000 // 어떤 합리적 grace보다 큰 델타(유예 만료 강제).
beforeEach(() => {
  vi.useFakeTimers()
})
afterEach(() => {
...
describe('IC1 — 활동 없는 done → 입력 스트림 스스로 종료', () => {
  it('턴1 완료 + 활동 없음 → 다음 pull이 done:true(닫힘) + events 자연 종료(abort 불필요)', async () => {
    ...
    // (LR4 P03) 유예 대비: 소비를 먼저 시작 → grace 만료 advance → 소비 완료 await.
    //   현재(즉시 close)에선 advance 전에 이미 닫혀 무해, 유예 코드에선 grace 만료로 닫힌다.
    const consume = (async () => {
      for await (const e of run.events) events.push(e)
    })()
    await vi.advanceTimersByTimeAsync(GRACE_EXPIRE_MS)
    await consume
...
describe('IC3 — 루프 소멸 후 다음 done → 종료', () => {
  it('턴1 CronCreate(유지) → 턴2 CronDelete(자율, push 없음) → 그 done에서 스스로 종료', async () => {
    ...
    // (LR4 P03) 유예 대비: 소비 먼저 → grace 만료 advance → 소비 완료 await(assert 불변).
    const consume = (async () => {
      for await (const e of run.events) events.push(e)
    })()
    await vi.advanceTimersByTimeAsync(GRACE_EXPIRE_MS)
...
describe('IC4 — idle-close 후 후속 턴이 resumeSessionId로 새 held-open을 연다', () => {
  it('세션1(무활동, idle-close) → session 이벤트로 sessionId 캡처 → 세션2가 resumeSessionId 전달 → options.resume 일치', async () => {
    ...
    // (LR4 P03) 유예 대비: 소비 먼저 → grace 만료 advance → 소비 완료 await(세션1 idle-close).
    ...
    // (LR4 P03) 세션2도 무활동 → idle-close: 소비 먼저 → grace 만료 advance → 소비 완료.
...
describe('Edge — interrupt 후 활동 없음: 다음 턴 경계(interrupt-result의 done)에서 닫힘', () => {
  it('interrupt-result(error suppressed)의 done 이후 pendingSends=0·활동 없음 → 다음 pull이 닫힘', async () => {
    ...
    // (LR4 P03) 유예 대비: interrupt-result done 이후 grace 만료 advance → 자연 종료.
    await vi.advanceTimersByTimeAsync(GRACE_EXPIRE_MS)
...
describe('Edge — 권한 대기 중(턴 내부)은 idle 판정 비대상', () => {
  it('permission_request 대기 동안 스트림이 끊기지 않고, respond 후 정상 완주 + 이후 idle-close', async () => {
    ...
    const events: AgentEvent[] = []
    // (LR4 P03) 유예 대비: 소비(응답 포함)를 먼저 시작 → done 이후 grace 만료 advance → 완료.
    //   권한 대기(turn 내부)는 done 전이라 grace 판정 비대상 — 응답 후 done까지 정상 완주하고,
    //   done 이후 무활동에서만 grace가 개입한다(계약 보존). 응답은 Promise 기반(타이머 무관).
    const consume = (async () => {
      for await (const e of run.events) {
        events.push(e)
...
describe('Edge — 멀티패널: idle-close 판정은 sessionKey/panel과 무관, 런별 독립', () => {
  it('무활동 패널A는 스스로 닫히고, 활동 있는 패널B는 A와 무관하게 유지된다', async () => {
    ...
    // (LR4 P03) 유예 대비: 패널A(무활동)는 grace 만료로 닫히므로 advance 필요.
    //   패널B(활성 크론)는 idle 판정 비대상 → 유예 미개입, turn2 후 자연 종료(advance 무해).
    await vi.advanceTimersByTimeAsync(GRACE_EXPIRE_MS)
    await drainA
    await drainB
