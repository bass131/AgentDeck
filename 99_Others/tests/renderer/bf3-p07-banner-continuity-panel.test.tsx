 ...
 *
 * 배경(01_Phases/07_BF3-backlog-sweep/07-banner-continuity.md): panelManagerStates(앱수명
...
describe('BF3 Phase 07 경계 ⓑ — panelManagerStates CAP(32) 축출 후 재마운트 시 activeLoops 소실 봉합', () => {
  it('activeLoops 보유(그러나 idle+unmount) 슬롯이 CAP 초과로 축출된 뒤 재마운트해도 activeLoops가 보존된다', async () => {
    ...
    // 1) loop-owner 슬롯: send → run 확보 → loops 이벤트로 activeLoops 세팅 → done으로 턴
    //    종료(idle 전이) → unmount.
    //    FB2(영호 육안 피드백 2026-07-04 ④) 봉합 이후: ADD_USER_MESSAGE가 단일챗
    //    sendMessage와 동형으로 isRunning을 낙관적으로 true 세팅하므로(패널 응답 대기
    //    인디케이터 표시), "턴 사이 idle"은 이제 done 이벤트로 명시적으로 도달해야 한다
    //    (이전엔 send() 자체가 isRunning을 건드리지 않아 우연히 false로 남아있었을 뿐 —
    //    실제로는 "응답 대기 중"을 "idle"로 오인한 것이었다). done은 activeLoops를
    //    건드리지 않는다(reducer/lifecycle.ts handleDone) — SDK 크론은 턴 완료 후에도 계속
    //    산다는 이 테스트의 전제 그대로 유지된다.
    ...
    expect(owner.result.current.state.isRunning).toBe(false) // done 이후 idle — activeLoops는 보존
    act(() => {
      owner.unmount()
    })
