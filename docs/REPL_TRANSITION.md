...
| (3) cron-turn 라우팅 + abort 분리 | ⏳ 잔여 | — | origin/pending-send은 (1)에 구현됨. 남음: **interrupt IPC 채널**(AGENT_INTERRUPT + run-manager.interrupt + preload — abort=세션종료/interrupt=턴중단 분리). (5)와 함께 사용됨 |
...
| (5) 렌더러 | ⏳ 잔여(**UI 사인오프 필요**) | — | REPL 모드 토글 + **currentRunId 세션스코프 재정의**(persistent run의 runId=sessionKey 고정 → cron-turn이 `panelSession.ts:258` 필터 통과) + cron-turn 휘발 렌더 + LoopIndicator(session_crons) 재배선 + lazy 멀티. **미적/동작 사용자 사인오프 대상** |
