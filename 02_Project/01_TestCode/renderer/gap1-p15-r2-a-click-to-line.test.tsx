...
vi.mock('../../../02_Project/00_Source/renderer/src/features/viewer', () => {
...
import { SearchResultView } from '../../../02_Project/00_Source/renderer/src/features/conversation/SearchResultView'
import { FileModal } from '../../../02_Project/00_Source/renderer/src/features/file'
...
const CONTENT_RESULT: AgentEventSearchResult = {
  ...
  matches: [
    ...
    { path: '02_Project/00_Source/renderer/src/App.tsx', text: '라인 정보 없는 매치' },
  ],
  total: 3,
}
...
const FILES_RESULT: AgentEventSearchResult = {
  ...
  files: ['02_Project/00_Source/main/a.ts', '02_Project/00_Source/main/b.ts'],
  total: 2,
}
...
describe('GAP1 P15 R2-A — SearchResultView 매치 클릭 → 라인 전달 (RED)', () => {
  it('매치 라인(라인 있음) 클릭 → openFile(path, undefined, line) — 3번째 인자로 라인 전달', () => {
    ...
    expect(call[0]).toBe('02_Project/00_Source/renderer/src/App.tsx')
  ...
  it('라인 정보 없는 매치 클릭 → line 미전달(기존 거동 핀)', () => {
    ...
    expect(spy).toHaveBeenCalledTimes(1)
    expect(spy.mock.calls[0][0]).toBe('02_Project/00_Source/renderer/src/App.tsx')
  ...
  it('파일 헤더 클릭 → line 미전달(기존 거동 핀)', () => {
    ...
    const header = container.querySelector(
      '[data-search-file="02_Project/00_Source/main/index.ts"]'
    ) as HTMLElement
    expect(header).toBeTruthy()
    fireEvent.click(header)

    expect(spy).toHaveBeenCalledTimes(1)
    expect(spy.mock.calls[0][0]).toBe('02_Project/00_Source/main/index.ts')
    expect(spy.mock.calls[0][2]).toBeUndefined()
  })
  ...
  it('파일 목록 모드(files_with_matches) 행 클릭 → line 미전달(기존 거동 핀)', () => {
    ...
    expect(spy).toHaveBeenCalledTimes(1)
    expect(spy.mock.calls[0][0]).toBe('02_Project/00_Source/main/b.ts')
    expect(spy.mock.calls[0][2]).toBeUndefined()
  })
})
