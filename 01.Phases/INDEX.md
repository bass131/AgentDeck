...
>
> **리네임 예정 안내**: 영호 결정(2026-07-11)으로 폴더를 시간순 `NN_` 접두(00~14)로 리네임할 예정이다. 아래 "예정 폴더명" 열은 **승인 대기 매핑 초안** — 실제 폴더 리네임은 **아직 미실행**이며, 이 문서는 목차+매핑 초안만 담는다.

---

## 마일스톤 표 (최초 커밋일 순)

| # | 현재 폴더명 | 예정 폴더명 | 기간(최초~최종 커밋) | 상태 | 한 줄 요약 |
|---|---|---|---|---|---|
...
- **실행 시점**: 폴더 `NN_` 접두 리네임은 **LR4 P03 커밋 후 별도 배치**로 실행한다(현재 P03 coordinator가 `LR4-session-stability` 폴더를 읽으며 진행 중 — 리네임이 경로를 흔들면 안 됨).
