...
| (4a) app-close closeAll | ✅ | `(이 커밋)` | `before-quit` → `disposeAllRuns()` → `RunManager.closeAll()`(활성 run 전부 abort→세션스코프 크론 동반 사망, 좀비0). TDD closeAll(3: 좀비0·빈경우·멱등). **reviewer 통과(CRITICAL 0)** |
