...
- [ ] **P01 완료** — stuck 재현 테스트 확보. P02·P03과는 독립(renderer) — 병렬 가능.

---

## 📝 작업 내용

- [ ] **(a) interrupt 기반 복구 — 우선** — `interruptRun`(`runtime.ts:280-287`)이 대상 run 생존을 확인(currentRunId 유효·main 응답 부재) → 죽었으면 `abortRun`처럼 isRunning=false 로컬 정리. 이 경로를 1차로 채택.
- [ ] **(b) send-guard 탈출 — 보수적 2차** — `sendMessage`의 `if(isRunning)return`(`runtime.ts:77`) 우회는 liveness 오판 시 **이중 send/고아 run 위험**이라 2차로만. 느린-하지만-살아있는 세션을 죽은 것으로 오탈출시키지 말 것(정확한 생존 판정 전제, 미확신 시 미채택).
- [ ] **(c) 재현 green** — P01 stuck 재현 테스트를 green으로 전환.

---

## ✅ 완료 조건

- [ ] interrupt 죽은 run→isRunning 정리 테스트 PASS
- [ ] 정상 interrupt(살아있는 run) 회귀 0
