 ...
 *
 * 검증 범위 (작업 A: MultiWorkspace 저장 / 활성세션 로드 / 언마운트 flush):
 *   (P1) 저장 보존: 디스크에 세션 A·B, 활성=A에서 패널 변경 후 upsert → A·B 둘 다 존재, A 갱신
 *   (P2) 활성세션 로드: store activeMultiSessionId='B' → 마운트 시 B 세션 패널 복원(A 아님)
 *   (P3) 언마운트 flush: 디바운스 진행 중 언마운트 → multiCmdUpsert 호출됨
 ...
 *
 * RMW1-P04(ADR-031): 저장(디바운스/flush)은 multiCmdUpsert(명령 1발) 경유 — main이
 * read→upsert→write를 원자 블록으로 실행한다. mock은 helpers/multiCmdMock.ts를 통해
 * main의 실제 순수 병합 함수(upsertSession)를 재사용 — "save 인자"를 직접 검사하던 옛
 * 단언들은 이제 병합 후 `_disk`(최종 디스크 상태)를 직접 검사하는 형태로 치환됐다.
 *
 * 아키텍처 준수:
 *   - window.api.multiSessionLoad + multiCmd* 경유만 (fs/Node 직접 0)
...
describe('P1 — RMW 보존: 활성 세션 저장 시 다른 세션 비소실', () => {
  ...
  it('디스크에 세션 A·B, 활성=A에서 디바운스 save → A·B 둘 다 존재', async () => {
    ...
    // multiCmdUpsert가 호출됐다면 병합 후 디스크(_disk)를 검사 — upsert 요청 자체는
    // 활성 세션(A) 한 건만 담지만, main의 upsertSession은 id 일치 세션만 교체하므로
    // 최종 디스크에는 B가 무손상 보존돼야 한다(ADR-031 — "read한 스냅샷 전체를 다시
    // 쓰기"가 아니라 "명시한 필드만 병합").
  ...
  it('upsert 후 디스크의 activeSessionId = 현재 활성 세션 id', async () => {
    ...
    // upsertSession은 activeSessionId를 절대 건드리지 않는다(ADR-031 — create/select
    // 전용) — 초기 시드값('sess-A')이 그대로 보존돼야 한다.
    if (mockMultiCmdUpsert.mock.calls.length > 0) {
...
describe('G1 — 유령 세션 방지: activeMultiSessionId 빈 문자열 → save 미발화', () => {
  ...
  it('activeMultiSessionId=""로 마운트 + 디바운스 발화 → multiCmdUpsert 호출 안 됨', async () => {
    // 디스크에 세션이 있어도 activeId 빈 문자열이면 upsert 명령 no-op
    _disk = makeDisk([{ id: 'sess-A', count: 2 }], 'sess-A')
    ...
    // buildActiveSession → performUpsert 가드에서 no-op(디바운스 타이머 자체가 안 걸림)
    await renderMultiWorkspace('')
    ...
    // activeId 빈 문자열이므로 upsert 미발화 (유령 'main-session' append 차단)
  ...
  it('activeMultiSessionId 채워지면 정상 저장 (기존 P1 보완)', async () => {
    ...
    // activeId 있으면 upsert 명령 발화 — main이 원자 병합 후 디스크(_disk)에 반영.
    expect(mockMultiCmdUpsert).toHaveBeenCalled()
    // 유령 'main-session' 아닌 실제 id로 저장됐다(병합 후 최종 디스크로 검증).
    expect(_disk?.activeSessionId).toBe('sess-A')
