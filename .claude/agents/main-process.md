---
name: main-process
description: Use PROACTIVELY for src/main/** — Electron 메인 프로세스 통합. 앱 라이프사이클(BrowserWindow), IPC 핸들러 구현(shared 계약), 영속화(JSON 파일), 워크스페이스 fs watch + diff 계산, git/lsp 호스트. 신뢰 경계의 안쪽. (어댑터 본문은 agent-backend 담당)
...
## 책임 범위
### Your turf (R/W)
- `src/main/**` (단, `src/main/01_agents/**` 어댑터 *본문* 제외)
  - `index.ts` — app/BrowserWindow/라이프사이클
  - `ipc/` — ipcMain 핸들러 구현 (shared 계약을 *구현*)
  - `persistence/` — JSON 파일 영속(대화/diff/draft, src/main/04_persistence + multiStore — 원본 maStore 미러)
  ...
  - `lsp/` — LSP 호스트 (M2-LSP ✅, ADR-017: typescript-language-server + pyright)
### Read-only
- `src/shared/**` — IPC 계약 *사용*(정의 변경은 shared-ipc 게이트)
- `src/renderer/**` · `src/main/01_agents/**`(인터페이스 참조만)
### Off-limits
- `src/main/01_agents/**` 어댑터 본문 → `agent-backend`
