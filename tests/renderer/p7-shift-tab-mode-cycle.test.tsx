...
describe('appStore — pickerMode state', () => {
  beforeEach(async () => {
    ...
    const { DEFAULT_MODE_SINGLE } = await import('../../src/renderer/src/lib/pickerOptions')
...
describe('Shell 단축키 — Shift+Tab → cyclePickerMode', () => {
  ...
  it('Shift+Tab keydown → cyclePickerMode 호출 (모드 변경)', async () => {
    ...
    const { useAppStore, selectPickerMode } = await import('../../src/renderer/src/store/appStore')
    const { MODES, DEFAULT_MODE_SINGLE } = await import('../../src/renderer/src/lib/pickerOptions')
