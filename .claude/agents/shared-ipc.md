---
name: shared-ipc
description: Use PROACTIVELY for src/shared/** + src/preload/** — main↔renderer 공유 계약. IPC 채널명/요청·응답 타입, 공통 AgentEvent 타입, preload contextBridge 노출. 계약은 *정의*만(구현은 main-process). 변경은 양쪽 영향 = trust-boundary/backend-contract 깃발.
...
### Read-only
- `src/main/**` · `src/renderer/**` — 계약 *사용처* 점검(정합 확인).
### Off-limits
...
4. **`AgentEvent` 변경 = backend-contract 깃발** — 전 어댑터·소비자 영향. agent-backend·renderer·qa 정합 동반(coordinator 조율). append 우선, breaking 변경은 신중.
