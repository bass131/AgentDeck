...
test('updateAvailable이면 "새 엔진 버전" 팝업이 실 버전으로 뜨고 확인으로 닫힌다', async () => {
  const info = await page.evaluate(() => window.api.checkEngineUpdate())
  ...
  // 부트 트리거가 set-dialog "새 엔진 버전"을 띄울 때까지 대기(네트워크 fetch 후).
  ...
  console.log('[engine-update] 팝업 메시지:', msg)
  expect(msg).toContain(info.current as string)
  expect(msg).toContain(info.latest as string)

  await page.screenshot({ path: join(SHOT_DIR, 'engine-update-notice.png'), fullPage: false })

  // "확인" 클릭 → 닫힘
  await dialog.locator('.sd-go', { hasText: '확인' }).click()
