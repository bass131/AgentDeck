...
renderer Worker. reviewer 무조건(복잡). (P05 드롭 — P01-(c) 실측)
순서: 01→04→02→**03**→07 (02의 AUTO가 선행 — /loop을 SDK로 넘겨도 크론이 생존하는 토대).

## ✔ 완료 기록 (2026-07-03)

- 삭제: loopCommand.ts·usePanelLoop.ts·loop 슬라이스 + 전용 테스트 4파일(단일·멀티 대칭).
  grep `isLoopCommand|LOOP_MAX_TICKS` 소스 0건. **LR2-03 봉합(abortRun activeLoops:[]·
  panel CLEAR_LOOPS)은 잔존**(reviewer 실측 — 함정 항목 준수).
- 부속: replMode 기본 true(system.ts) + prefs 왕복(main.tsx 복원·Shell.tsx 저장 — 기존
  채널 재사용, 키 부재 시 true 폴백). LoopStatusBanner sdk 변형만(셀렉터 계약 유지).
- 게이트: typecheck 0 · lint 0 · test **3880 green**(전용 테스트 삭제 반영) · build OK ·
  reviewer 🟢(🔴0·🟡2 정보성 — loop-context-live 삭제 정당·OFF 원샷은 의도된 열화).
- **라이브 PASS**(11.2s): `/loop 1m` → ToolSearch→CronCreate→loops→배너→정지→소멸.
  선행 실패 2회는 진단 결과 **모델 재량 플레이크**(무도구 직답 — 앱·엔진 경로 결백을
  백엔드-직접 probe 2회 + raw 이벤트 로거로 실측 확정) → 스펙에 retries:2 + 진단 로거 상설.
