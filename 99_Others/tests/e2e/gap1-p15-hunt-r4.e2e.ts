 ...
 *       - [X3-c 활성 하이라이트 관찰] 활성 셀 정적 하이라이트(.sag-cell--active — TG1 P08,
 *         옛 flex-grow=2 확대 폐기) 관측 여부 — 타이밍 의존(짧은 서브 턴이면 미관측 가능)이라
 *         annotation(하드 단정 X).
...
interface SplitSample {
  ...
  queueText: string
  /** 활성 셀 정적 하이라이트(.sag-cell--active) 존재 여부(TG1 P08 — 옛 flex-grow=2 확대 폐기). */
...
async function sampleSplitView(page: Page): Promise<SplitSample> {
  return page.evaluate(
    ({ gridCell, queueStrip }) => {
      ...
      // 활성 하이라이트(TG1 P08 — 옛 flex-grow=2 확대 폐기): 정적 하이라이트 클래스 .sag-cell--active 존재 여부.
