...
- [x] `npm run test` green (패널 cwd 유/무 × run·팔레트 PASS)
- [x] `npm run lint` 0 problems
- [x] **영호 육안** — 패널별로 다른 프로젝트의 커맨드 목록이 표시됨(FB2 때 증상의 최종 봉합) 확인. loop_track: human-visual — 기능은 진행하고 육안 검토 병행(무인 commit X).
- [x] **send는 P15 선해결로 superseded(문자적용 반려), 실제 갭=팔레트 root — reviewer 🟢 실증**
