...
describe('gap1-p08 Grep content 모드 → search_result(matches 파싱)', () => {
  it('content 문자열(경로:라인:텍스트) 파싱 → tool_result 뒤에 search_result 추가 방출', () => {
    const content = [
      "02_Project/00_Source/main/index.ts:10:import { app } from 'electron'",
      '02_Project/00_Source/main/index.ts:42:app.whenReady()',
      '02_Project/00_Source/renderer/src/App.tsx:7:export function App()',
    ].join('\n')
    const obj = userToolResultMsg({
      ...
      toolUseResult: {
        mode: 'content',
        numFiles: 2,
        filenames: ['02_Project/00_Source/main/index.ts', '02_Project/00_Source/renderer/src/App.tsx'],
        content,
        numLines: 3,
        numMatches: 3,
      },
  ...
  it("컨텍스트 구분줄 '--'·빈 줄은 skip — 매치 2건만 파싱", () => {
    const content = '02_Project/00_Source/a.ts:5:match one\n--\n\n02_Project/00_Source/b.ts:9:match two\n'
    const obj = userToolResultMsg({
      ...
      toolUseResult: {
        mode: 'content',
        numFiles: 2,
        filenames: ['02_Project/00_Source/a.ts', '02_Project/00_Source/b.ts'],
        content,
        numLines: 4,
        numMatches: 2,
      },
    ...
    expect(mapClaudeStreamLine(obj)).toEqual<AgentEvent[]>([
      expectedToolResult('toolu_grep_sep_01', [{ type: 'text', text: content }]),
      {
        ...
        mode: 'content',
        matches: [
          { path: '02_Project/00_Source/a.ts', line: 5, text: 'match one' },
          { path: '02_Project/00_Source/b.ts', line: 9, text: 'match two' },
        ],
...
describe('gap1-p08 Grep files_with_matches 모드 → search_result(files·total)', () => {
  it('filenames → files 그대로 · total=numFiles · matches 없음', () => {
    const obj = userToolResultMsg({
      ...
      blockContent: [
        { type: 'text', text: 'Found 3 files\n02_Project/00_Source/main/a.ts\n02_Project/00_Source/main/b.ts\n02_Project/01_TestCode/c.test.ts' },
      ],
      toolUseResult: {
        mode: 'files_with_matches',
        numFiles: 3,
        filenames: ['02_Project/00_Source/main/a.ts', '02_Project/00_Source/main/b.ts', '02_Project/01_TestCode/c.test.ts'],
      },
    })
...
describe('gap1-p08 Grep count 모드 → search_result(total=numMatches 우선)', () => {
  it('numMatches 있음 → total=numMatches · files=filenames · matches 없음(content 미파싱)', () => {
    const obj = userToolResultMsg({
      ...
      blockContent: [{ type: 'text', text: '02_Project/00_Source/a.ts:12\n02_Project/00_Source/b.ts:5' }],
      toolUseResult: {
        mode: 'count',
        numFiles: 2,
        filenames: ['02_Project/00_Source/a.ts', '02_Project/00_Source/b.ts'],
        content: '02_Project/00_Source/a.ts:12\n02_Project/00_Source/b.ts:5',
        numLines: 2,
        numMatches: 17,
      },
    })
  ...
  it('numMatches 없음(SDK optional) → total=numFiles 폴백', () => {
    const obj = userToolResultMsg({
      ...
      blockContent: [{ type: 'text', text: '02_Project/00_Source/a.ts:3\n02_Project/00_Source/b.ts:1' }],
      toolUseResult: {
        mode: 'count',
        numFiles: 2,
        filenames: ['02_Project/00_Source/a.ts', '02_Project/00_Source/b.ts'],
...
describe("gap1-p08 Glob → search_result(mode:'glob')", () => {
  it('GlobOutput(mode 필드 없음 — 형상 판별) → files=filenames · total=numFiles · truncated 전달', () => {
    const obj = userToolResultMsg({
      ...
      blockContent: [{ type: 'text', text: '02_Project/00_Source/main/index.ts\n02_Project/00_Source/preload/index.ts' }],
      toolUseResult: {
        durationMs: 12,
        numFiles: 2,
        filenames: ['02_Project/00_Source/main/index.ts', '02_Project/00_Source/preload/index.ts'],
        truncated: false,
      },
    })
    const events = mapClaudeStreamLine(obj)
...
describe('gap1-p08 폴백 — search_result 무방출(기존 tool_result 거동 불변)', () => {
  ...
  it('Grep/Glob 형상 아님(파일편집 출력 {filename, patch, ...}) → search_result 무방출', () => {
    const obj = userToolResultMsg({
      ...
      toolUseResult: {
        filename: '02_Project/00_Source/main/index.ts',
        status: 'modified',
        additions: 3,
        deletions: 1,
        changes: 4,
  ...
  it('isReplay:true + Grep 형상 → [] (GAP1 P04 S-13 가드 유지 — search_result도 재방출 금지)', () => {
    const obj = userToolResultMsg({
      ...
      toolUseResult: {
        mode: 'files_with_matches',
        numFiles: 1,
        filenames: ['02_Project/00_Source/a.ts'],
      },
      isReplay: true,
    })
    expect(mapClaudeStreamLine(obj)).toEqual<AgentEvent[]>([])
