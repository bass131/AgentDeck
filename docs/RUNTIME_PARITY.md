...
#### ✅ Phase A 설계 확정 (2026-06-24, plan-auditor + Plan 에이전트 병렬 검증·수렴)
**확정 결정**:
1. **messageId 소스 = (B) stateful 펌프 카운터**(`ClaudeAgentRun`이 `_curTextId`/`_blockSeq`/`_launchTag`로 부여, 원본 `engine.ts:153 nextBlockId`+LAUNCH_TAG 미러). **(A) SDK `message.id`는 함정** — 우리 `includePartialMessages=false`라 한 `message.id`가 `[text][tool][text]` 블록에 걸쳐 재사용→reducer가 도구 경계 가로질러 1버블 병합(인터리브 깨짐). 매퍼 `mapClaudeStreamLine`는 **순수 유지**(messageId 미부여), 펌프가 `text` 이벤트 후처리 태깅(이미 `_recordFilePending`/`_handleTaskToolCall` 동일 패턴 L680-698). 계약상 `AgentEventText.messageId?` **optional**(펌프가 항상 채움, 미부여 시 단일버블 degrade=회귀 아님).
2. **영속 화해 = 단일 thread 렌더 + save 시 파생**. 라이브 진실=`thread: ThreadItem[]` 하나. `messages: ConversationEntry[]`는 직렬화 뷰로 격하(save 시 `thread.filter(kind==='msg')`→`{role,content}[]`, load 시 역). toolgroup 미영속(MVP·과거대화=텍스트만). **done 시 streamingText→messages 확정 dance 3곳(appStore:718·panelSession:93·reducer) = 수정 아닌 *삭제***(text가 도착 즉시 thread msg로 들어가므로 확정 불필요). done은 isRunning=false·usage만.
3. **인터리브 reducer 듀얼 포인터**: `text`→openGroupId=null(다음 도구=새 그룹)+openMsgId=messageId, `tool_call`(parent 없음)→openMsgId=null(다음 텍스트=새 버블)+그룹 open/append, `tool_result`→thread 내 in-place. 원본 `session.ts:222-290` 미러. subagent/parentToolId 분기·todos·changedFiles·fileDiffs·pending* 불변.
4. **🔴 차단 결함(auditor 포착, Plan 누락→보강)**: `MultiWorkspace.tsx` PanelView가 `messages,streamingText` 직접 렌더(L393-394·493-499)+hasContent 계산 → reducer 코어 변경과 **같은 커밋(A-2)에서 동반** 안 하면 6패널 컴파일 깨짐. `PanelMessage[]`는 thread로 흡수.
5. **ADR 불요**(backend-contract 깃발 선례: Phase21 done.contextWindow·M4 이벤트들 모두 ADR 없이 agent-events 헤더 주석+reviewer로 진행). `agent-events.ts` 헤더 갱신+reviewer 필수.

**측정가능 AC(auditor 요구)**: ① reducer 단위 `text→tool_call→text`→thread `[msg,toolgroup,msg]`(msg 2분리) PASS ② 연속 tool 2개 같은 그룹+사이 text 오면 새 그룹(openGroupId=null) ③ 펌프 단위 `[text][tool][text]`→messageId 분리/연속 text→동일 ④ save→load round-trip 후 thread msg 항목 동일(toolgroup만 의도 소실) ⑤ 라이브 e2e `.thread` 자식 순서 msg→toolgroup→msg 교차 DOM 검증 ⑥ typecheck 양쪽 green + 단위 28파일 마이그레이션 green.

**3 sub-step 분할(각 사이 reviewer)**:
- **A-1** (shared-ipc + agent-backend): `AgentEventText.messageId?` 추가 + 헤더 깃발 주석 + 펌프 `_curTextId`/blockId 태깅(text 후 tool_call·메시지경계서 reset). 펌프 단위테스트(AC③). → typecheck+단위 green.
- **A-2** (renderer 코어, 한 커밋): `threadTypes.ts` 신규(ThreadItem union: msg/thinking/toolgroup/notice, cmdresult 의도제외 주석) + reducer `thread` 모델 전면 재작성 + appStore(done dance 삭제·send/save/load thread 화해·셀렉터) + panelSession(PanelMessage→thread 흡수) + **MultiWorkspace 동반**. reducer/panel 단위(AC①②④). → typecheck green.
- **A-3** (view + e2e): `ToolGroup.tsx` 신규(ToolCallCard 래퍼+lead 아바타 원본 App.tsx:988) + Conversation 3단렌더→단일 `thread.map` + SmoothMarkdown/자동스크롤 deps `[thread,isRunning]` 재배선 + Composer history `thread.filter(user)`. conversation 단위 + 라이브 e2e(AC⑤). → 통합 reviewer.

#### 🏁 Phase A 완료 (2026-06-24, 로컬 master·push 0)
- **A-1 ✅** (`03afe79`): backend 펌프 messageId(`_curTextId`/per-run `_launchTag`·blockSeq, 원본 nextBlockId 미러). 매퍼 순수 보존. message-id-pump.test 12·reviewer 통과.
- **A-2 ✅** (`399e28b`): reducer `thread` 단일 스트림 + 듀얼 포인터(openGroupId/openMsgId). streamingText·toolCards·PanelMessage **완전 제거**(단일 진실원, main이 Worker 과대보고분 직접 정리). appStore.messages는 thread-파생 영속 투영. panelSession·MultiWorkspace 동반(차단결함 보강). ToolGroup.tsx/.css 신규. 테스트 10파일 이행 + thread-interleave 15. reviewer 통과·차단 0. typecheck·vitest 2676·build green.
- **A-3 ✅** (`bee71a8`): ToolGroup.css 원본 기하 충실도(42px 가터·lead-ava 절대배치) + WorkingIndicator 모달 억제 게이트. **라이브 e2e 실측(LIVE_SDK 4/4 PASS)**: `.thread` 자식 순서 `[user][assistant][toollog][assistant][toollog][assistant]` — Claude Code식 text→tool→text 시간순 인터리브 확정(평면 목록 아님). Phase B diff(+4 −0) 인터리브 안에서 보존.
- **AC 전부 충족**: ①②(reducer 인터리브 단위)·③(펌프 messageId 분리)·④(round-trip msg 보존)·⑤(라이브 DOM 순서)·⑥(typecheck+단위 green). 🟡 이월(비차단): WorkingIndicator thinking-during-live 미세차·messages 투영 4곳 향후 selectThread 단일화·lead 아바타 라이브 미관측(조건부 발화, 소스 검토 정합).
→ **사용자 요청 2건(턴별 인터리브 A + 도구 diff B) 모두 완료.** 대화 렌더가 원본 Claude Code 스타일로 전환됨.
