...
- [ ] **#2 엔진 버전 업데이트 팝업**(진행 예정): 원본 = `EngineGate.tsx` 부트 useEffect가 `engine.state()`+`engine.listAvailable()`(npm `https://registry.npmjs.org/@anthropic-ai/claude-agent-sdk` fetch·dist-tags.latest) 병렬 조회→`cmpVer(active,latest)<0`이면 "새 엔진 버전 / 현재 X / 최신 Y로 업데이트?" 프롬프트(나중에/업데이트). 업데이트→`install()`(npm install `@pkg@ver` → `~/.agentcodegui/engines/<ver>`)→`setActive`(config.json)→다음 부트 `loadActiveQuery()` 동적 import. **난이도 분해**: (a)체크+팝업=작음(npm조회+cmpVer+다이얼로그) / (b)설치=큼(멀티버전 폴더) / (c)동적로드=최대(ClaudeCodeBackend 적응). **우리 현황**: `engine-state.ts` version=하드코딩 "0.3.186"(→package.json 읽기 필요), `AppUpdateGate.tsx` 셸 있으나 'available'(새버전 프롬프트) phase 없음(downloading/downloaded/error만), EngineGate는 우리가 인증게이트로 재목적화. **착수 = (a) 먼저**(체크+팝업), (b)/(c)는 사용자와 범위 협의. 새 ADR(예 ADR-018 엔진버전관리)은 사용자 판단.
