...
| 5 | P07 이 **커밋돼 있는가** (B 세션 기준 — 개정 2026-07-27) | `git log --oneline -3` · `git status --porcelain` | `fe03430`(refactor(naming) P07)이 이력에 있고 워킹트리 **깨끗** |
...
⚠️ **조건 5 개정 (2026-07-27, B 세션 기동 게이트 불일치 후)** — 초판은 *"P07 분이 미커밋 상태 — 정상이다"* 였다. 그것은 **A 세션(커밋 전 감사) 시점의 기대값**이고, A 종결 후 P07 이 `fe03430` 으로 커밋되면서 §6 을 추가할 때 **이 행을 함께 갱신하지 않았다.** B 세션이 이 불일치로 무수정 중단한 것은 **게이트가 설계대로 동작한 것이다 — 결함 아님.** (같은 문서 안에서 §6 은 갱신하고 §0 은 안 한 것 — *"축으로 나누면 축의 교차점이 빈다"* 의 문서판이다.) 여전히 유효한 부분: **`02_Source`·`00_Documents`·`01_Phases` 는 커밋하지 마라** — §4 제약 ① 그대로, Codex 가 커밋할 수 있는 것은 자신의 `.codex/**` 변경뿐이다.
