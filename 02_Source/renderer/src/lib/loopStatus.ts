 ...
 *
 * 판정 소스 2종: SDK 크론 루프(LoopInfo[], main CronTracker가 loops 이벤트로 정규화)와
 * goal(=`/goal` — SDK stop-hook 자기지속, 크론 아님). goal은 크론과 메커니즘이 달라
 * (loops 이벤트 0, 자체 진행 컨텍스트로만 관측) 별도 인자로 받는다.
 *
 * **단일 표시 불변식**: 이 함수 한 곳에서만 우선순위를 결정 —
