...
- [x] **P03 완료** — P03이 goal 진행/종료 생존신호를 방출(P05가 소비할 신호 소스가 준비됨).

---

## 📝 작업 내용

- [x] **(a) 배너 결속(소비)** — renderer `loopStatus`가 **P03이 방출하는 goal 진행/종료 신호(autonomy_status)**를 소비해 배너 on/off(active→표시 / ended→해제). "선행 active 없는 ended 무시" 방어 포함. 신호 *방출*은 이 Phase 소관이 아니다(P03이 담당).
- [x] **(b) 폴백** — 터미널 리셋(abort·dead-run·error) 폴백으로 신호 유실 시 배너 stuck 방지. (heartbeat/watchdog 폴백은 REPL 4b auto-revive 재도입 시 협의 — P06 인계 리스크로 pin 등록.)

---

## ✅ 완료 조건

- [x] goal 배너 실상태 정합 테스트(조기발동 억제·종료 해제·조용한 사멸 시 해제) PASS
- [x] `npm run typecheck` (main+renderer) 0 errors · `npm run test` green · `npm run lint` 0 problems
- [x] reviewer CRITICAL 0(renderer-only, 깃발 없음)
