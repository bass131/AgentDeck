...
describe('maskSecrets()', () => {
  it('export 존재 확인', async () => {
    const mod = await import('../../../02_Project/00_Source/main/07_engine/engineVersions')
    expect(typeof mod.maskSecrets).toBe('function')
  })

  it('_authToken=abc → 마스킹', async () => {
    const { maskSecrets } = await import('../../../02_Project/00_Source/main/07_engine/engineVersions')
...
describe('installVersion() — semver 검증 (spawn 미호출 보장)', () => {
  ...
  for (const [version, errRe] of rejected) {
    it(`버전 ${JSON.stringify(version)} → 즉시 거부, spawn 0`, async () => {
      const { installVersion } = await import('../../../02_Project/00_Source/main/07_engine/engineVersions')
...
describe('setActive() → loadActiveQuery()', () => {
  it('setActive(null) → 주입된 userData 에만 기록 + loadActiveQuery() = null', async () => {
    const { setActive, loadActiveQuery } = await import('../../../02_Project/00_Source/main/07_engine/engineVersions')
    const ud = newUserData()
  ...
  it('setActive(설치된 버전) → config 기록 + getVersionState.active 반영', async () => {
    const { setActive, getVersionState } = await import('../../../02_Project/00_Source/main/07_engine/engineVersions')
    const ud = newUserData()
  ...
  it('setActive(미설치 버전) → throw, config 미기록', async () => {
    const { setActive } = await import('../../../02_Project/00_Source/main/07_engine/engineVersions')
    const ud = newUserData()
...
describe('loadActiveQuery()', () => {
  it('active 없음(config activeVersion:null) → null 반환', async () => {
    const { loadActiveQuery } = await import('../../../02_Project/00_Source/main/07_engine/engineVersions')
    const ud = newUserData()
...
describe('getVersionState()', () => {
  it('설치 없음 → installed=[], active=null', async () => {
    const { getVersionState } = await import('../../../02_Project/00_Source/main/07_engine/engineVersions')
