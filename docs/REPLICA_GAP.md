...
- **M4 멀티·대화고도화** (Phase 20~): **M4-1 ✅**(model/effort/mode→claude CLI 인자 매핑[run-args allowlist] + 토큰 게이지 실데이터[done.usage÷MODEL_CONTEXT_WINDOW]) → **M4-2**(슬래시 실행·@mention 해석·이미지 첨부 저장·큐 드레인) → **M4-3**(멀티 6패널 동시실행·세션 CRUD) → **M4-4**(권한/질문 응답). 잔여 항목: usage 분석(히스토리/비용), 실시간 per-turn context(`--include-partial-messages` 조사).
