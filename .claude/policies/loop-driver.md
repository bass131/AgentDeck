...
### 3-1. 야간 자율 런 (night-run) — v1.5 (영호 승인 2026-07-28)

v1의 변형이다: **기동은 attended**(영호가 자기 전 터미널에서 명시 시작), **지속은 무인**. v2(예약·cron 기동)와 다르며 v2는 여전히 defer다. 근거 = 성향 29(게이트 강도는 손상의 가역성으로) — 밤에 허용되는 것은 전부 가역(전용 브랜치 로컬 커밋)이고, 비가역은 전부 게이트에 남는다.

- **범위**: work-judge 버킷 **(a) 기계 판정**만. (b) 육안·(c) 비가역은 아침으로 이연 — 로컬 커밋까지, push·PR 없음(CORE-06 불변). human-gate Phase는 영호가 기동 전 명시 GO한 것만 포함하며, GO는 Phase 문서에 날짜와 함께 박는다.
- **적용 대상**: 현재 /work-run 프리셋만. refactor-sweep의 apply 모드는 자체 실증(v1 승격) 전이라 밤 대상이 아니다.
- **실패 규칙** (Second Brain 성향 도출, 2026-07-28):
  1. 게이트 red면 **접근을 바꿔 재시도 1회**(같은 입력 반복 금지 — 성향 24). 그래도 red면 그 Phase를 스킵한다.
  2. 스킵 시 **체크포인트 박제**(성향 18): 실패 diff를 patch 파일로 저장 + 재현 커맨드 + 게이트 출력 요약. 작업 트리는 그 Phase가 만진 파일 한정으로 되돌려 다음 Phase와 격리한다.
  3. 스킵 사유는 **층위 귀속**(성향 26): 계획 결함 / 구현 실수 / 게이트·환경 문제를 구분해 기록 — 실행 실수를 방법론 한계로 포장하지 않는다.
  4. **판정기 불변**(성향 17): 밤에는 게이트·훅·기존 테스트 단언을 실패 우회 목적으로 수정하지 않는다. red를 green으로 만드는 유일한 길은 입력(코드) 수정이고, 그게 안 되면 스킵이다.
  5. **누적 캡**(성향 15·19): 스킵 2건 누적 시 밤 런 전체를 중단하고 상태를 보존한다 — 연속 실패는 개별 Phase가 아니라 공통 토대 문제의 신호다.
- **질문 금지**: AskUserQuestion 대신 위 스킵 절차. 아침 보고 = 완료/스킵/중단 전수 + 체크포인트 좌표 + 커밋 목록.
- 무인 배치 금지의 나머지는 전부 유효 — 이 절이 허용하는 것은 "영호가 시작한 런이 그가 잠든 뒤에도 계속되는 것"뿐이다.
...
- **비가역 사람 게이트는 절대 보존** — 훅 축②(`dangerous-cmd-guard`)와 `permissions.ask` **2층 모두** ([`pr-and-merge-gate.md`](pr-and-merge-gate.md)). 루프가 어느 한 층이라도 약화시키면 위반. ⚠️ 축② 처분 = 실행 직전 사람 승인 강제(CORE-06 v3, 2026-07-29 — 구 v2는 항상 차단 + 영호 `!` 직접). 루프는 명령을 호출하되 영호가 다이얼로그에서 승인해야 실행되고, 거부되면 실행 없이 그 지점을 사람 게이트 정지로 처리한다.
...
범용 드라이버를 도메인에 맞게 구체화한 *프리셋*들:

- **refactor-sweep** ([`../commands/refactor-sweep.md`](../commands/refactor-sweep.md)) = *refactor 프리셋*. 핫스팟(churn×복잡도)·스멜 2축 진단(기본 dry-run) → `--apply` 회차만 Worker 위임 → 회귀 게이트 → 재검증 → 리포트. 안전 규칙은 그 문서의 "스윕 불변식" + 정본 포인터(CORE-06·07, ADR-003)로 소유가 갈린다.
- **work-run** ([`../skills/work-run/SKILL.md`](../skills/work-run/SKILL.md)) = *Phase 실행 프리셋*. 미착수 Phase(`status:pending`) → 의존성 정렬 → Worker → reviewer(깃발 무조건) → 회귀 게이트 → Phase별 commit, 버킷 (c)/(b) 정지. [`../skills/work-plan/SKILL.md`](../skills/work-plan/SKILL.md)(분해)의 실행 짝.

둘 다 *도메인 무관 드라이버*의 구체화. **안전 가드는 한 줄도 약화 X.**
