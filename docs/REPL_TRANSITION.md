...
| (4a) app-close closeAll | ✅ | `d62fc88` | `before-quit` → `disposeAllRuns()` → `RunManager.closeAll()`(활성 run 전부 abort→세션스코프 크론 동반 사망, 좀비0). TDD closeAll(3: 좀비0·빈경우·멱등). **reviewer 통과(CRITICAL 0)** |
| (4b) watchdog auto-revive | ❌ **드롭(사용자 결정 2026-06-26)** | — | "끄면 죽어야 한다. 끈 뒤에도 도는 건 버그." 맥락 복원 = 자동 부활이 아니라 **다음 프롬프트의 resume**(session_id 영속 `773285c`/`bab1e2f`, 이미 빌드). 좀비 오토리바이브 리스크 회피 + 사용자 통제 모델. → **빌드 안 함.** |
...
| (5c) "loop 진행중" 표시기 | ✅(**dev 사인오프**) | `02fa9b4`·`f2727a6` | 데이터원=CronCreate tool(프로브: session_crons는 undef). 백엔드 Cron 추적→중립 `loops` 이벤트. 렌더러 LoopRunningIndicator(우측상단 "loop 진행중-작업내용"+회전) + gloss. TDD loop-tracking(12)+indicator(12). cron-turn 프레임 연출은 `7f15bca` |
| (5d) 정지 버튼 + 런어웨이 수정 | ✅ | `9b20418` | 표시기 × 정지=세션 abort(크론=세션스코프→사멸→LLM호출 중단)+빈 loops. 표시기 비스크롤 레이어화·gloss 강화. 지속펌프 finally도 빈 loops push |

**🔬 CronDelete 프로브(`delete-probe.mjs`)**: 채팅 "루프 종료해줘" → Claude `CronDelete {id:"<cronId>"}` 호출(CronCreate id와 일치) → tool_result "Cancelled job <id>". → **채팅 종료도 끝까지 동작**(크론 정지 + `_handleCronDelete`가 `input.id` 추출→`_activeLoops` 제거→빈 loops→GUI 제거). 테스트 픽스처도 `{id}` 정합. **reviewer 🟡②(CronDelete 키 best-effort) 해소.** 두 정지 경로: ① 채팅 "종료"(CronDelete, 세션 유지) ② × 버튼(abort, 세션 종료·resumable).

### 🛠️ npm run dev 실측 후속 수정 (2026-06-26)
| 이슈 | 커밋 | 수정 |
|---|---|---|
| 재시작 후 **멀티 패널 맥락 0** | `bab1e2f` | `PanelThreadSnapshot.sessionId?` 영속+복원(Phase1.5 멀티 누락분). 단일챗은 정상(session 캡처 `_processSdkMessage:1255`) |
| 답변 폰트(앱폰트 미적용)·**흰배경** | `07d4033` | `.content` serif→`--font-sans`. MarkdownView bg-0→transparent(코드블록 배경 유지) |
| 루프 gloss가 **본문 가림** | `07d4033` | gloss 상단 깊은 글로우 제거 → 테두리 액센트 링만(본문 비가림) |
| **맨아래로** 버튼 없음 | `07d4033` | ScrollToBottomButton(바닥서 위로 스크롤 시 표시)+isScrolledUp 순수함수 |
| 입력창 **자동높이** | `07d4033` | computeComposerHeight(1~3줄 신축, 초과 overflow:auto) |
| 표시기 스크롤 시 묻힘 | `9b20418` | 비스크롤 부모(.conversation/.ma-p-body)로 오버레이 레이어화 |

**프로브 자산(gitignore)**: rearm/origin/watchdog/crons/crons2/delete-probe.mjs — REPL 런타임 ground truth.
