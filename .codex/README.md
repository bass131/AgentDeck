...
4. `node .codex/harness-doctor.mjs --live` — `STATIC: PASS` + `HOOK-GUARD: PASS` + `OS-READ-BOUNDARY: UNENFORCED_EXPECTED` + `LIVE-CONFORMANCE: ACCEPTED_WITH_LIMITATION` 확인. exit 3(`REVALIDATION_REQUIRED`)이면 CLI 버전이 baseline(0.144.0)과 달라진 것 — 읽기 deny 실태를 재실측하고 doctor BASELINE과 ADR-033을 갱신합니다.
