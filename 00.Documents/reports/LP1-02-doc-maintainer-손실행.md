...
> **미확인 17건의 성격**: D2의 과거 gate 테스트 스냅샷 11건(재현 = 과거 커밋 체크아웃 필요, 읽기 전용 트랙 밖) + 라이브 SDK 실행 결과 2건 + gitignore 프로브 아티팩트 1건 + 원본/미구현 특성 주장 2건 + `.claude` 접근 금지 1건. 억지 판정을 피한 결과이지 문서 오류가 아니다.
