...
## 6. 심층 감사 결정 (UltraCode 워크플로 `harness-cleanup-audit`, 2026-06-26)
4 병렬 감사(ClaudeDev·AgentDeck 하네스 + docs + 정리) → 합성. 합성이 정리 감사 오판을 재실측 정정.

### 하네스 보강 — 포트/스킵/적응 (H1~H3 DONE 이후)
| # | 항목 | 결정 | 이유 | 적용 |
|---|---|---|---|---|
| H4 | `phase-gate-validator` 훅 (-DONE 강제) | **적응** | 솔로라 5필드 과함 → **복잡↑ Phase만** `_milestone-DONE.md`(5단계), advisory(exit0)→안정 후 exit2 | 훅=scripts/hooks 직접 / settings 등록=스테이징 |
| H5 | `grade-and-risk`/`subagent-routing`/`review-tiering` 정책 3파일 | **부분 포트** | 등급/깃발이 CLAUDE.md+_routing에 산재 → `.claude/policies/` 신설(3개만) | ADR-025 + .claude deny → 사용자 적용 |
| H6 | work-judge 3버킷(기계/육안/비가역) | **적응** | refactor-sweep G3/G7에 암묵 존재 → grade-and-risk.md에 명문화 | H5에 통합 |
| H7 | knowledge 캐시 + GC | **스킵** | 솔로+AI self-reinforcement 위험, MEMORY.md가 대체 | — |
| H8 | 팀 namespace(per-owner) | **스킵** | 솔로 overhead | — |
| H9 | `/engine:goal` 루프 드라이버 | **스킵** | 내장 /loop 활용이 1순위(MEMORY loop-intent) | — |
| H10 | `/cross-review`(Codex β) | **스킵** | 듀얼백엔드는 Track 2 미착수 | — |
| H11 | **하네스 보강 ADR 박제(ADR-025)** | **가져옴** | H1~H3가 ADR 공백 → 헌법 "결정=ADR 근거" | ADR 초안 → 사용자 확정 |

→ **잔여 보강 우선순위**: H11(ADR-025) → H4(phase-gate advisory) → H5+H6(정책 3파일). H7~H10 스킵 확정.

### 대규모 정리 — 실행/정정
- **STAGE 0 ✅ 실행**(2026-06-26): `out/`·`*.tsbuildinfo`·`artifacts/*.html|*.png` 삭제(전부 git 미추적, 재생성 가능). `.mjs` 프로브 15개 보존(ground truth). ~10M+ 청소.
- **정리 감사 오판 정정**(합성 재실측): `composerSampleData`(테스트 3곳 사용)·`f14SampleData`(Shell.tsx)·`run-args.ts`(신뢰경계 allowlist, 5참조) = **keep, 삭제 금지**. → 향후 delete는 `git grep`+`git ls-files` 재실측분만.
- **거대파일**(보류, refactor-sweep 진단 대상): ClaudeCodeBackend 2472(G7 영구제외)·ipc-contract 2290·appStore 1642·MultiWorkspace 1582(G3)·ipc/index 1501.
- **docs**: 루트=활성+권위만. `ORCHESTRATION_FIX.md`(ADR-021 완료) → archive 후보(단 ADR.md 참조 갱신 필요 → 사용자 게이트). 나머지 드라이버는 활성 keep.

### 로드맵: STAGE0 정리✅ → STAGE2 하네스 제안(H11→H4→H5/H6, .claude 게이트) → STAGE3 refactor-sweep --dry-run → STAGE4 docs 정리. push/배포=인간 게이트.

## 5. 진행
- **H1-a CHANGELOG ✅**(`.claude/CHANGELOG.md`) · **H1-b/c 훅 ✅**(risk-detector·reviewer-auto-trigger 등록+smoke) ·
  **H2 refactor-sweep ✅**(`.claude/commands/refactor-sweep.md` — `/refactor-sweep` 스킬 등록) · H3(5단계 DONE) **보류**.
- 적용: `.claude/**` deny 일시 해제(사용자 인가) → 직접 적용 → **작업 후 deny 복원**(하네스 재보호).
- 검증법: 임의 `src/shared/ipc-contract.ts` 편집 시 `🚩 risk-detector` + `🔍 reviewer-auto-trigger` 출력(세션 재시작 후 확실).
  `/refactor-sweep --dry-run`으로 v0 진단(commit 0) 먼저 권장.
