...
| 19 | — (신설) | `19_LM1-live-model-switch` | 2026-07-17 | ✅ done | REPL 지속세션 라이브 모델 전환 — GAP1 P13 setPermissionMode 7단 체인을 SDK `Query.setModel`로 미러 + 모델 vs 모드 비대칭 봉합(역통지 부재 → 반영 정본 = 다음 assistant `message.model`) + effort GUI 모델 정합 편입(P06 shared 승격·P07 피커 반응형). 7 Phase(LM1-DONE). Vitest 5325·TDD 64케이스. PR #24(A트랙)·#25 당일 머지(9877605). *상태 정정 2026-07-28 — 표가 착수 전 "5 Phase pending" 시점에 멈춰 있었음.* |
