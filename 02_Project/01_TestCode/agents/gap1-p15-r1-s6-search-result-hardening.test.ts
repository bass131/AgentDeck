...
describe('GAP1 P15-R1 S6a — tool_use_result 다중 블록 귀속 골든 (안전망)', () => {
  const GREP_FWM = {
    ...
    numFiles: 1,
    filenames: ['02_Project/00_Source/main/index.ts'],
  }
...
describe('GAP1 P15-R1 S6b — Grep 라인번호 없는 출력 오파싱 방어 (RED)', () => {
  it('`-n:false` 형식(경로:텍스트)에서 텍스트 내 `:숫자:` 우연 매치 → filenames 대조로 드롭, 유효 0이면 무방출', () => {
    const content = '02_Project/00_Source/server.ts:listen on localhost:3000:ok'
    const events = mapClaudeStreamLine(
      userMsgWithBlocks({
        ...
        toolUseResult: {
          mode: 'content',
          numFiles: 1,
          filenames: ['02_Project/00_Source/server.ts'],
          content,
          numLines: 1,
        },
      })
