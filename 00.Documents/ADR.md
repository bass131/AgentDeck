...
1. **권한 프로필** — root Supervisor는 `:danger-full-access`를 기본으로 사용하고, 점검 역할은 `agentdeck-readonly`, 구현 Worker는 `agentdeck-workspace`, secretary는 운영 경로 중심 `agentdeck-operations`를 사용한다. 세 SubAgent 프로필은 모두 `.env*`와 `secrets/**` 읽기를 거부한다. Full Access root의 비밀 파일 금지는 헌법·Hook·execpolicy·사람 게이트로 유지한다. Codex 보호 경로인 `.codex/state/**`는 operations profile에 write 승격하지 않고 secretary가 갱신안을 반환하면 root가 반영한다.
...
6. **정적/실행 검증 분리** — `harness-doctor`의 role/model/permission/digest/bridge 정적 PASS는 파일 정합만 뜻한다. custom agent 실제 모델·권한 적용은 trusted 새 세션의 live acceptance 전까지 PENDING이다.
