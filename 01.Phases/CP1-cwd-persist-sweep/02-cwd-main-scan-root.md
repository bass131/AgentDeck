...
- [ ] `npm run test` green (폴백 4케이스 PASS)
- [ ] `npm run lint` 0 problems
- [ ] reviewer(trust-boundary 필수) CRITICAL 0
- [ ] 검증 통과 root는 반영, 실패·부재는 전역 폴백됨을 테스트로 입증
- [ ] **roots 레지스트리 멤버십 불요 판정 근거 명문화** — 스캔은 `.claude/skills|commands` 하위 한정 직접 읽기(`skills.ts:311`)로, 기존 전역 root와 동일 신뢰 수준의 구조 검증(절대경로·존재·디렉토리)으로 충분. 별도 roots 레지스트리(fs.read 게이트) 등록 불요.
- [ ] **스캔 범위 `.claude` 하위 한정 불변식 테스트** — root 파라미터가 `.claude/skills|commands` 밖을 읽지 않음을 입증.
