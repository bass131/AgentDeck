...
import { makeInitialState, applyAgentEvent } from '../../src/renderer/src/store/reducer'
...
describe('RU-A: REPL 토글 버튼 store 계약', () => {
  ...
  it('A-1: replMode 기본값 true (기본 ON)', async () => {
    // 5a 완료로 이미 replMode:true가 기본값
    expect(useAppStore.getState().replMode).toBe(true)
  })

  it('A-2: selectReplMode 셀렉터가 replMode를 반환', async () => {
    const { selectReplMode } = await import('../../src/renderer/src/store/appStore')
