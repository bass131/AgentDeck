...
status: done (§7 실측 종결 — 진짜 원인 확정·수정 fa9df22 + 구조적 원인[폴백 부재]→ADR-029)
date: 2026-07-01
summary: §1-6은 flush/held-open 가설 추적(반증). §7이 실제 원인 확정 — 단일채팅 CONVERSATION_SAVE 핸들러 sessionId drop(fa9df22 수정) + resume-only 구조라 transcript 폴백 부재(ADR-029로 처방). ⚠️ §1-6의 "flush 갭" 가설은 반증됐으니 인용 주의.
---
...
## §8. ★★ 최종 반전 — resume은 작동, 진짜 잔여 증상 = 모델의 거짓 disclaimer (2026-07-02, 라이브 확정) ★★

fa9df22 이후 영호가 "그래도 실앱에서 이전 대화 못 가져온다" 재보고 → 실앱 디스크 포렌식 + 라이브 격리 probe로 종결.

### 결정적 실측 (4소스)
1. **영호 실세션 레코드** `agentdeck/chats/ab233f39.json`: `sessionId="60c6aef2-…"` (유효 UUID) — **fa9df22 실앱에서 작동**(sessionId 저장됨).
2. **SDK 세션 저장소** `~/.claude/projects/C--Dev-Test-Project/60c6aef2.jsonl` 존재 — resume이 읽을 트랜스크립트가 디스크에 있음(SDK가 resume 데이터를 실제 영속).
3. **그 .jsonl 40줄 흐름**: "안녕(ROOT)→HS1123 기억요청→[Read/Write/Edit로 memory 파일 저장]→(재시작)→이전대화 기억해?→'전체 내용은 기억 못 함'" 이 **압축·요약 없이 한 세션에 연속.** ⇒ 모델이 disclaimer를 뱉는 순간 msg 2·10(안녕·HS1123)이 **컨텍스트에 그대로 있었음.**
4. **라이브 격리 probe** (`99.Others/tests/e2e/lr1-resume-isolation-probe.e2e.ts`): fresh userData/workspace + "기억해" 미사용 + 도구금지 + 직접질문 → 재시작 후 코드네임 `ZEBRA49QX` **정확히 회상**, memory 파일 히트 **0** ⇒ 회상 출처 = **resume 단독**. (영호 실앱 수동 재현도 "나비" 직접회상 성공.)

### 결론
- **resume은 완벽히 작동한다.** 재시작·날짜변경을 넘어 맥락을 복원한다(fa9df22 + 기존 배선).
- 영호가 본 "이전 대화 기억 못 함"의 잔여 증상 = **모델의 거짓 disclaimer.** 학습된 "나는 과거 대화 기억 못 함" 멘트를, 컨텍스트가 있어도 **메타질문**("이전 대화 기억해?")에 반사적으로 뱉는다. **직접질문**("아까 코드네임 뭐였지?")엔 정확히 답한다.
- **혼선 2겹**: (i) memory 도구 — "기억해줘" 시 모델이 `~/.claude/projects/<slug>/memory/*.md`에 사실을 파일 저장(`settingSources:['user','project','local']`가 로드). "기억한 숫자:1"은 며칠 전 세션 잔재. (ii) 메타질문 disclaimer. 영호 실테스트가 이 둘을 동시에 밟아 "안 된다"처럼 보였다.

### 오진 정정 (2차)
ADR-024 "held-open 증발" 전제가 틀렸던 것처럼, **LR1 "앱이 대화를 잊는다" 전제도 (fa9df22 이후로는) 틀렸다.** 핵심 버그는 이미 닫혔다. 잔여는 영속이 아니라 **모델 자기보고/UX**.

### 영호 결정 (2026-07-02) → 마일스톤 잔여 재조정
- **(a) systemPrompt에 "대화 연속성" 주입** — resumeSessionId 있을 때 "보이는 이전 메시지는 이 사용자와의 실제 대화·앱이 복원한 것, 기억으로 취급하고 기억 못 한다 말하지 마(단 컨텍스트에 없는 건 지어내지 마)". 메타질문 probe로 억제 측정. (agent-backend, backend-contract → reviewer)
- **(b) "맥락 복원됨" 배지** — 모델 말과 무관하게 앱 상태를 UI로 신뢰(renderer, ui-visual → 영호 육안).
- **Phase 03(견고성: session이벤트 즉시저장·cwd 안정화) → 백로그 이연** (관측된 문제 아닌 엣지 하드닝).

### 교훈
- **모델 자기보고 ≠ 시스템 진실.** "기억 못 한다"는 모델 말을 버그 신호로 오독. 디스크·probe로 교차검증해야 했다.
- **사용자 실사용 경로를 그대로 재현하라.** e2e는 늘 직접질문이라 disclaimer를 안 밟았고, 영호는 메타질문+memory도구를 밟았다 — 그 간극이 "TDD만 통과" 인상을 만들었다.
