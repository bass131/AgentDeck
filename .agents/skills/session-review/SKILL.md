...
`.codex/state/current-pin.txt`, 현재 Worktree의 Git diff와 status를 기준으로 하며, Claude runtime state는 읽기 폴백 외에는 변경하지 않습니다. 점검은 상태 보고이며 사용자가 요청하지 않은 코드 수정이나 비가역 작업으로 확장하지 않습니다.
