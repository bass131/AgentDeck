/**
 * lr4-p03-idle-grace.test.ts — 자율반복 idle-close 유예/상한/생존신호 계약 (LR4 Phase 03)
 *
 * 배경(설계 스펙): 지속세션 펌프(`claudeAgentRun.ts` `_runPersistentPump`)는 turn 경계에서
 * "살아있을 이유"(pending user turn·활성 루프)가 없으면 즉시 idle-close하던 것을, "짧은
 * 유예(grace) 후 판정 + 무한루프 상한(cap) + 생존신호(autonomy_status) 방출"로 바꾼다.
 * agent-backend가 유예/상한/신호를 구현했고, 이 스위트는 그 계약(5종)을 고정한다.
 *
 * 앱 소스는 이 Phase에서 건드리지 않는다(테스트만 — qa 영역).
 *
 * ── 결정성(determinism) 원칙 : 비중첩 barrier(GraceProbe) ─────────────────────────
 * 유예는 진짜 단일 `setTimeout(IDLE_CLOSE_GRACE_MS=3000)`(claudeAgentRun `_scheduleIdleGrace`)
 * 이므로 가짜 타이머(`vi.useFakeTimers()` + `vi.advanceTimersByTimeAsync`)로 제어한다.
 *
 * **핵심 불변식(BL1-P02 재구성)**: 어떤 순간에도 `advanceTimersByTimeAsync` 호출은 정확히
 * 하나만 in-flight여야 한다. 예전 구조는 mock 제너레이터가 *제너레이터 내부에서* clock을
 * 진행(`await vi.advanceTimersByTimeAsync(...)`)해, 테스트 본문의 바깥 advance와 중첩(nested)
 * 됐다. step-splitting(100ms 재스케줄) 시절엔 각 델타가 작아 우연히 버텼지만, production이
 * 단일 setTimeout으로 정리되자 중첩 advance가 3000ms 타이머를 통째로 점프시켜 관측 순서가
 * 무너졌다(계약3·4 RED).
 *
 * 해법 = **GraceProbe barrier**(아래). mock 제너레이터는 절대 스스로 clock을 진행하지 않고
 * `await probe.checkpoint()`로 "여기 도달했다"만 신호하고 park한다. clock 진행은 테스트 본문
 * (`driveCheckpoints`) 한 곳에서만 순차로 일어난다 → 중첩 0, 결정적.
 *
 * grace(3000)에 결속하지 않는 델타 상수:
 ...
 *
 * ── 상수(constants) ──────────────────────────────────────────────────────────
 * agent-backend가 `IDLE_CLOSE_GRACE_MS`·`MAX_CONSECUTIVE_AUTONOMOUS_TURNS`를 export하지만
 * 값 하드코딩(영호 확정 2026-07-11) + "agent-backend 상수와 일치해야 함" 주석 유지 — 이 스위트는
 * 정확한 grace(3000)에 결속하지 않으므로 상수 결합을 최소화한다.
 *
...
// ── GraceProbe : 비중첩 barrier (재사용, counter 기반 — lost-wakeup 방지) ─────────────
//
// 목적: mock 제너레이터와 테스트 본문 사이의 clock 진행을 "한 곳(본문)"으로 모으는 랑데부.
//   - 제너레이터: `await checkpoint()`로 도달을 알리고 release()될 때까지 park.
//   - 테스트 본문: `waitForCheckpoint()`로 다음 도달을 기다렸다가(이미 도착했으면 즉시),
//     clock을 GRACE_PROBE_MS만큼 순차 진행한 뒤 `release()`로 제너레이터 1개를 재개.
//
// counter 기반인 이유(lost-wakeup 방지): checkpoint()가 waitForCheckpoint()보다 먼저 도착해도
// arrivedCount로 기록되므로, 뒤늦게 부른 waitForCheckpoint()가 그걸 즉시 소비한다(놓치지 않음).
//
// 불변식: 어느 순간에도 in-flight advance는 0개(제너레이터는 clock을 만지지 않는다). 이 barrier
// 덕분에 advance는 오직 driveCheckpoints/테스트 본문에서만, 정확히 순차로 일어난다.
...
class GraceProbe {
  ...
  /** 제너레이터: checkpoint 도달 — release()될 때까지 park. */
  ...
  /** 테스트: 다음 checkpoint 도달까지(이미 도착해 있으면 즉시) 대기. */
...
/**
 * 비중첩 barrier 드라이버 — 제너레이터의 checkpoint를 테스트 본문 한 곳에서 순차 구동한다.
 *
 * 각 순회: (1) 다음 checkpoint 도달 대기(또는 consume가 먼저 끝나면 종료 감지),
 * (2) GRACE_PROBE_MS 만큼 clock 진행(유예 3000ms 미만 — 흡수 창 재현), (3) park된 제너레이터
 * 1개를 재개.
 *
 * `bound`는 그 테스트의 mock 스크립트가 방출할 수 있는 checkpoint의 **고정 상한**이다(무한대기
 * 방지 — 메모의 함정: Promise.race를 무한루프로 감싸면, 제너레이터가 checkpoint 없이 실제
 * 3000ms 만료를 기다리는 구간에서 아무도 clock을 진행 안 시켜 hang). consume(세션 소비)가 먼저
 * settle하면(예: cap-reached로 입력 스트림이 닫혀 세션 자연종료) 즉시 break한다.
 *
 * 호출 후 잔여 유예 만료는 테스트 본문이 EXPIRE_MS 델타를 1회 순차 advance해 마무리한다(이제
 * 비중첩이라 안전).
 */
async function driveCheckpoints(probe: GraceProbe, consume: Promise<void>, bound: number): Promise<void> {
  for (let i = 0; i < bound; i++) {
    ...
    if (!gotCheckpoint) break // 세션 자연 종료(예: cap-reached로 입력 스트림 닫힘)
    await vi.advanceTimersByTimeAsync(GRACE_PROBE_MS)
    await Promise.resolve() // pull.then microtask(closed 감지 등) 정착 보장
    probe.release()
  }
}
...
afterEach(() => {
  // 테스트 간 타이머 상태 누출 방지(afterEach 복원 — Phase 함정 3).
  vi.useRealTimers()
})
...
// 생존 의존"으로 모델링한다:
//   - 즉시 close(옛 코드): turn1 done → idle-close 즉시 발동 → 2번째 input pull이 즉시
//     done:true → closed=true → mock이 turn2 안 냄 → done 1개(생존 기대와 배치).
//   - 유예 코드: 2번째 input pull이 park(grace 보유) → closed=false → turn2 방출 → 생존 →
//     done ≥2 + active 방출(GREEN).
//
// (BL1-P02) 제너레이터 내부 advance를 `probe.checkpoint()`로 치환하고, 고정 1회 루프
// (bound=1)로 정규화해 회귀(중첩 advance 재발)를 원천 차단한다.
...
describe('계약1 — goal 자멸 재현: 유예가 입력을 park시켜 자율 continuation 흡수(다중 스텝 생존)', () => {
  it('turn1 done 이후 continuation이 유예 창에 흡수돼 turn2까지 진행(done ≥2 + active)', async () => {
    ...
    const queryFn: QueryFn = async function* (p) {
      ...
      // (BL1-P02) 여기서 clock을 만지지 않는다 — checkpoint로 park하고, 테스트 본문이
      // GRACE_PROBE_MS(<grace)만 순차 진행한다. 즉시 close(옛 코드)면 pull이 done:true로
      // 해소(closed=true), 유예 코드면 grace가 park시켜 미해소(closed=false).
    ...
    // 고정 1회 checkpoint(turn2 흡수)만 순차 구동 후, 잔여 유예 만료를 EXPIRE_MS로 마무리.
...
//
// (BL1-P02) 이 계약의 mock은 애초에 clock을 만지지 않는다(`await inputIter.next()` 직접 대기
// 뿐) → 중첩 없음 → barrier 불필요. 변경 없음(top-level 단일 EXPIRE_MS advance 유지).
...
describe('계약3 — 상한: 연속 자율 턴이 상한을 넘으면 유계 강제종료 + ended(cap-reached)', () => {
  it('101 연속 자율(cron-origin) 턴 구동 → 무한이 아닌 유계 종료 + ended(cap-reached)', async () => {
    ...
    //
    // (BL1-P02) 제너레이터 루프 내부 advance를 `probe.checkpoint()`로 치환한다. checkpoint 상한
    // (bound)은 이 mock이 아는 고정 상한 = attempts. cap 강제종료로 입력 스트림이 닫히면
    // (closed=true) 제너레이터가 return → consume settle → driveCheckpoints가 break(무한대기 X).
    ...
    // 각 자율 턴 사이 checkpoint를 attempts 상한까지 순차 구동(cap 발동 시 자연 종료로 break).
  ...
  it('자율 턴 몇 개 → 사용자 push 개입 → 카운터 리셋(다시 자율 여유 확보)', async () => {
    ...
    //
    // (BL1-P02) 자율 루프 내부 advance를 checkpoint로 치환(bound=preAutonomous). push 개입은
    // 소비 루프(consume)에서 그대로 유지되고, 자율 루프 뒤 tail(inputIter.next 직접 대기)은
    // clock을 만지지 않으므로 그대로 둔다 — 잔여 유예 만료는 EXPIRE_MS로 마무리.
    let pushed = false
    ...
    const queryFn: QueryFn = async function* (p) {
      ...
      // (tail: clock 미접촉 직접 대기 — 잔여 유예 만료는 테스트 본문 EXPIRE_MS가 처리.)
    ...
    // 사용자 push가 반영돼 user origin done이 2개 이상 도달해야 한다(카운터 리셋으로 세션 생존).
...
//
// (BL1-P02) 제너레이터 내부 advance를 checkpoint로 치환하고 고정 1회 루프(bound=1)로 정규화.
...
describe('계약4 — 신호 방출 정합: active(흡수) → ended(reason) 순서 + reason 리터럴 계약 일치', () => {
  it('goal 세션: active가 ended보다 먼저 방출되고 ended.reason이 계약 리터럴과 일치', async () => {
    ...
    // turn2 흡수(1회 checkpoint) 후 다음 유예 만료까지 진행.
    await driveCheckpoints(probe, consume, 1)
    await vi.advanceTimersByTimeAsync(EXPIRE_MS)
    await consume
...
//
// (BL1-P02) 이 계약의 mock은 clock을 만지지 않는다(순수 push 기반, `await inputIter.next()`
// 직접 대기 뿐) → 중첩 없음 → barrier 불필요. 변경 없음.
//
