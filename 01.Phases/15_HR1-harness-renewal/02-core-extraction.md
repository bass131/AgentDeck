...
- [ ] 코어 조항에 안정 clause ID·버전 부여(예: CORE-01 신뢰경계 v1) + machine-readable manifest 작성(`00.Documents/harness/core-manifest.json` — 조항별 Claude/Codex 구현·검증 지점 선언) — 어댑터 정합을 사람 눈이 아니라 기계가 검사할 기반 (Codex adversarial #4)
...
- [ ] 전 코어 조항이 clause ID·버전·양 엔진 매핑을 가짐 — 미매핑 조항 0 (conformance 게이트 기계화는 P06)
