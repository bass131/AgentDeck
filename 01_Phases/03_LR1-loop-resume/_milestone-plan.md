...
status: pending
grade: 대규모
summary: 영호 실불편("옛 대화 이어가면 기억 못 함")을 resume 신뢰성 + transcript 폴백(ADR-029)으로 해결. loop·replMode 전환은 LR2로 분리(영호 버그와 무관 확정). 5 Phase.
---
...
**영호 결정(2026-07-01)**: 하이브리드 — resume 주수단 + resumeSessionId 없을 때 **최근 N토큰 transcript 폴백**(ADR-029 초안).

### scope 명시 (plan-auditor 점검용)
- **범위 안**: ① sessionId 저장 수정(완료) ② transcript 폴백(ADR-029) ③ resume 견고성(session이벤트 즉시저장·sessionKey·cwd) ④ UI 맥락배지 ⑤ 통합 e2e + 문서 정정.
...
- **ADR**: ADR-029(transcript 폴백)가 ADR-013(Claude Code resume 순수 충실)에서 의도적 이탈 → 영호 사인오프·커밋 필요.
...
| 02 | transcript 폴백 (ADR-029 핵심) | 복잡 | agent-backend | auto-gate | backend-contract | 01 + ADR-029 사인오프 |
...
> **known-gap(plan-auditor)**: resume "성공했으나 빈 세션"(만료·손상)은 폴백 트리거(sessionId 유무)로 안 잡혀 조용한 기억상실 잔존 → ADR-029 §미해결로 이연, Phase 05 문서에 "sessionId 있음 ≠ 맥락 복원" 1줄 명시.
...
- 문서(ADR-029·REPL_TRANSITION·ADR-024·FEATURE_MAP) = 영호 단독(AI 초안 → 영호 커밋).
