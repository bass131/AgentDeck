...
test.describe('M7 탐색기 lazy 스케일링 — node_modules 포함 대형 repo', () => {
  ...
  test('TC-3: 02_Project/00_Source 폴더 클릭 → 1레벨 children 즉시 로드', async () => {
    test.setTimeout(20_000)

    // 표준 레이아웃에서 소스는 02_Project 아래 한 단계 더 들어가 있다 — 부모부터 펼친다.
    const projectNode = page.locator('.fe-node.fe-dir-head[title="02_Project"]')
    await projectNode.waitFor({ state: 'visible', timeout: 8_000 })
    if ((await projectNode.getAttribute('aria-expanded')) !== 'true') {
      await projectNode.click()
    }

    const srcNode = page.locator('.fe-node.fe-dir-head[title="02_Project/00_Source"]')
    await srcNode.waitFor({ state: 'visible', timeout: 8_000 })

    const beforeCount = await page.locator('.fe-node').count()
    console.log(`[TC-3] 02_Project/00_Source 클릭 전 fe-node 수: ${beforeCount}`)
    ...
    const elapsed = Date.now() - t0
    console.log(`[TC-3] 02_Project/00_Source 펼침 후 fe-node 증가까지 ${elapsed}ms (< ${EXPAND_TIMEOUT_MS}ms)`)

    const afterCount = await page.locator('.fe-node').count()
    console.log(`[TC-3] 02_Project/00_Source 클릭 후 fe-node 수: ${afterCount} (증가: +${afterCount - beforeCount})`)
    expect(afterCount).toBeGreaterThan(beforeCount)

    const srcChildDirs = page.locator('.fe-node[title^="02_Project/00_Source/"]')
    const srcChildCount = await srcChildDirs.count()
    console.log(`[TC-3] 02_Project/00_Source 하위 fe-node 수: ${srcChildCount}`)
    expect(srcChildCount).toBeGreaterThan(0)

    console.log('[TC-3] 02_Project/00_Source lazy 펼침 확인: PASS')
  ...
  test('TC-5: 검색창에 "store" 입력 → 02_Project/00_Source 깊이의 파일이 결과에 포함', async () => {
    ...
    const hasSrcDeep = deepPathTexts.some(
      (p) => p.includes('02_Project/00_Source/') || p.includes('renderer') || p.includes('main')
    )
    const hasStoreFile = nodeNames.some(
      (n) => n.toLowerCase().includes('store') || n.toLowerCase().includes('app')
    )

    console.log(`[TC-5] 02_Project/00_Source/ 깊이 경로 포함: ${hasSrcDeep}, store 관련 파일명 포함: ${hasStoreFile}`)
    expect(
      hasSrcDeep || hasStoreFile,
      'listFiles IPC 기반 검색이 02_Project/00_Source/ 깊이 파일을 반환하지 않음'
    ).toBe(true)
