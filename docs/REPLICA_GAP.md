...
- **M4 멀티·대화고도화** (Phase 20~): **M4-1 ✅**(model/effort/mode 매핑 + 토큰 게이지) → **엔진 SDK 전환 ✅(ADR-016, Phase 21)**(claude -p CLI→@anthropic-ai/claude-agent-sdk `query()`, 원본 engine.ts 미러; 실 contextWindow 게이지 채택; 커밋 1c47d58/d52b139) → **M4-2**(슬래시 실행·@mention 해석·이미지 첨부 저장·큐 드레인) → **M4-3**(멀티 6패널 동시실행·세션 CRUD) → **M4-4**(권한/질문 응답). 잔여 항목: usage 분석(히스토리/비용), 실시간 per-turn context(`--include-partial-messages` 조사).
