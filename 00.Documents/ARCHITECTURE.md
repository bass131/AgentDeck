...
│   │       ├── lib/               # viewer.ts(확장자→뷰어 라우팅)·splitView.ts(SubAgent 스플릿 배정 정책 순수 함수, GAP1 P14) 등
...
type AgentEvent =
  ...
  // ── GAP1 확장 (P13) ──
  | { type: 'permission_mode' }     // 엔진 측 권한 모드 상태 관찰 신호 — 라이브 전환(agent.setMode)의 *결과* 정본(피커/배지 동기화). mode는 picker id 어휘(엔진 리터럴 어댑터 밖 누출 0)
```
...
### 4. 파일 변경 감지
main의 `02_fs/` watcher가 워크스페이스를 감시 + 에이전트 `file_changed` 이벤트와 대조 → "AI가 건드린 파일" 인디케이터 + diff(작업트리 vs 스냅샷) 계산. 어댑터 측 `fileChangeTracker`는 워크스페이스 root 보유 시 root 밖 경로(`..` 탈출·타 드라이브)를 `file_changed`로 방출하지 않는다(컨테인먼트, GAP1 P15 — root 미지정 tracker는 기존 거동 유지·과잉 필터 금지).
...
> P13 신규: 라이브 모드 전환 채널 `agent.setMode`의 runId·mode도 renderer발 untrusted string — main 핸들러가 화이트리스트 4종(`normal`·`plan`·`acceptEdits`·`auto`) 검증 후에만 `AgentRun.setMode()`로 전달한다(밖 값·SDK 어휘·임의 문자열 전부 거부 — 임의 문자열의 엔진 모드 주입 차단).
