...
describe('F8-01: 검색 필터', () => {
  ...
  it('검색어 입력 시 일치 세션만 표시된다', async () => {
    const { SAMPLE_SESSIONS } = await import('../../../02_Project/00_Source/renderer/src/lib/sidebarSampleData')
    const container = await renderSidebar()
...
describe('F8: scope 안전 검증', () => {
  it('sidebarSampleData는 window.api 참조가 없어야 한다(정적 상수)', async () => {
    const data = await import('../../../02_Project/00_Source/renderer/src/lib/sidebarSampleData')
