...
import {
  ...
  gitFileAt,
  gitWorkingFile,
} from '../../../02_Project/00_Source/main/02_fs/git'
...
describe('GIT_ROOT isAbsolute 가드', () => {
  ...
  it('GIT_ROOT 핸들러: 상대경로 cwd는 null 반환 시뮬레이션', () => {
    ...
    expect(handleGitRoot(repoDir)).not.toBeNull()
  })
})
...
describe('gitLog', () => {
  ...
  it('date는 unix milliseconds 숫자다', async () => {
    ...
    const commits = await gitLog(root!)
    for (const c of commits) {
      expect(c.date).toBeGreaterThan(1_577_836_800_000)
      expect(c.date).toBeLessThan(4_102_444_800_000)
    }
  })
