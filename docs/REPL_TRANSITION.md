...
- **빌드 순서**(plan-auditor 최종 게이트 후 확정): **(0)** shared 계약(`persistent`/`sessionKey` 중립 추가) →
  **(1)** run-manager 세션모델(done≠delete 옵트인 분기, sessionKey 장수 채널 — 단발경로 회귀 0) →
  **(2)** `PersistentSession`+Manager(held-open `query({prompt:AsyncIterable})`, push-queue) →
  **(3)** cron-turn 라우팅(pending-send 카운터+origin)+abort 분리(interrupt/closeSession) →
  **(4)** watchdog(실제 사망만 resume 재개, 1회 발동) → **(5)** 렌더러 REPL 토글+세션 라우팅+cron-turn 렌더+LoopIndicator 재배선(session_crons).
- 각 단계 독립 커밋·미push. ADR-003(엔진 리터럴 어댑터 내부)·신뢰경계(main 단독)·한국어 불변.
