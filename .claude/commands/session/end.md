...
>
> ⚠️ **CORE-06 v3(2026-07-29) — 이 절의 명령은 실행 직전 훅이 사람 승인을 강제한다.** `dangerous-cmd-guard` 축②가 GO 여부와 무관하게 `permissionDecision: "ask"` 다이얼로그를 띄운다(구 v2는 exit 2 차단 + 영호 `!` 직접 실행 — 완화 근거 = CORE.md v3 개정 블록). 따라서 아래 각 단계에서 AI가 할 일은 **정확한 명령을 조립해 호출하는 것**이고, 실행 여부는 영호가 다이얼로그에서 결정한다. 다이얼로그가 뜨는 것은 사고가 아니라 **설계대로 작동한 것**이다. 반대로 다이얼로그 없이 통과하면 그게 사고다(훅 사망 신호) — 즉시 중단하고 보고하라. `!` 프리픽스 사람 경로도 여전히 열려 있다.
...
#### 4-B. push (GO 후 — 호출하면 훅 ask 다이얼로그)
AI가 브랜치명을 채워 호출한다:
```bash
git push -u origin <현재 브랜치>
```
영호가 다이얼로그에서 승인하면 실행되고, 거부하면 실행되지 않는다(CORE-06 v3 정상 동작). 영호가 `! git push …` 로 직접 실행하는 경로도 여전히 유효하다.
...
- 있으면 → 00_Documents/CHANGELOG.md에 한 줄 추가하고 commit ([H]/[M]/[L])
- 없으면 → 스킵
```
