...
//   하드 게이트를 유지하면서 Red를 관측한다. 수리는 Phase 2 몫이며, 여기서 가드 본체는 고치지 않는다.
...
const ROWS = [
  ...
  { id: 'RM-03', cmd: 'rm -R build', moodie: 'deny', expect: 'deny', flag: true, note: '대문자 재귀 — Moodie 회귀 (Backlog 9)' },
  { id: 'RM-04', cmd: 'rm --recursive build', moodie: 'deny', expect: 'deny', flag: true, note: '긴 플래그 — Moodie 회귀 (Backlog 9)' },
  { id: 'RM-05', cmd: 'rm dist -rf', moodie: 'deny', expect: 'deny', flag: true, note: '후치 플래그 — Moodie 회귀 (Backlog 9)' },
  ...
  { id: 'RM-08', cmd: '"rm" -rf dist', moodie: 'deny', expect: 'deny', flag: true, note: '명령 인용 — 인용부 드롭 회귀 (Backlog 3·9)' },
  // ── 삭제류: git reset --hard (Moodie L47)
  { id: 'GR-01', cmd: 'git reset --hard origin/main', moodie: 'deny', expect: 'deny', flag: false, note: '작업 손실 기본형' },
  { id: 'GR-02', cmd: 'git reset "--hard"', moodie: 'deny', expect: 'deny', flag: true, note: '플래그 인용 — Moodie 회귀 (Backlog 9)' },
  { id: 'GR-03', cmd: 'git -C repo reset --hard', moodie: 'deny', expect: 'deny', flag: false, note: 'git 전역 옵션 접두' },
  { id: 'GR-04', cmd: 'git.exe reset --hard', moodie: 'allow', expect: 'deny', flag: true, note: '실행 파일명 머리 토큰 — 신규 표면 (Backlog 9)' },
  { id: 'GR-05', cmd: 'Git reset --hard', moodie: 'allow', expect: 'deny', flag: true, note: '대문자 머리 토큰 — 신규 표면 (Backlog 9)' },
  ...
  { id: 'CO-02', cmd: 'git checkout --', moodie: 'deny', expect: 'deny', flag: true, note: '경로 없는 `--` — Moodie 회귀 (경로 개수 조건 차이)' },
  { id: 'CO-03', cmd: 'git checkout .', moodie: 'allow', expect: 'deny', flag: true, note: '점 경로 — 신규 표면 (Backlog 9)' },
  ...
  { id: 'BR-04', cmd: 'git branch -d -f feature', moodie: 'allow', expect: 'deny', flag: true, note: '분리 동치 — 신규 표면 (Backlog 9)' },
  { id: 'BR-05', cmd: 'git branch -df feature', moodie: 'allow', expect: 'deny', flag: true, note: '소문자 결합 동치 — 신규 표면 (Backlog 9)' },
  { id: 'BR-06', cmd: 'git branch --delete --force feature', moodie: 'allow', expect: 'deny', flag: true, note: '긴 플래그 동치 — 신규 표면 (Backlog 9)' },
  { id: 'BR-07', cmd: 'git branch -d merged', moodie: 'allow', expect: 'allow', flag: false, note: '안전 삭제 — 오탐 회귀 불변식' },
  // ── 삭제류: git stash (Moodie 무규칙)
  { id: 'ST-01', cmd: 'git stash drop', moodie: 'allow', expect: 'deny', flag: true, note: '스태시 폐기 — 신규 표면 (Backlog 9)' },
  { id: 'ST-02', cmd: 'git stash clear', moodie: 'allow', expect: 'deny', flag: true, note: '스태시 전량 폐기 — 신규 표면 (Backlog 9)' },
  ...
  { id: 'NP-03', cmd: 'echo npm publish', moodie: 'deny', expect: 'allow', flag: true, note: '언급 오탐 — 머리 토큰 판정 미적용 (Backlog 1)' },
  ...
  { id: 'GH-03', cmd: 'echo gh pr create', moodie: 'allow', expect: 'allow', flag: true, note: '언급 오탐 — 정규식 잔존 (Backlog 1)' },
  // ── 영속 (Moodie L63)
  { id: 'CF-01', cmd: 'git config --global user.name x', moodie: 'deny', expect: 'deny', flag: false, note: '전역 설정 영속 변경' },
  { id: 'CF-02', cmd: 'git config --global --get user.name', moodie: 'deny', expect: 'allow', flag: true, note: '읽기 전용 오탐 (Backlog 10)' },
  ...
  { id: 'HD-02', cmd: "git commit -F - <<'EOF'\nrm -rf dist 회귀 수정\nEOF", moodie: 'deny', expect: 'allow', flag: true, note: '소비자=커밋 메시지 · 실행 소비자 없음 → 본문 오인 (Backlog 14, Phase 2 수리)' },
...
try {
  ...
  const liveHits = existsSync(liveLog)
    ? readFileSync(liveLog, 'utf8').split('\n').filter((l) => l.includes(SYNTHETIC.session) || l.includes(SYNTHETIC.agentId)).length
    : 0
