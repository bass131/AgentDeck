 ...
 *
 * 대상: 00_Documents/00_Harness/conformance-check.mjs
...
const SCRIPT = path.join(REPO_ROOT, '00_Documents', '00_Harness', 'conformance-check.mjs')
...
function makeFixture(spec: FixtureSpec): { root: string; parent: string } {
  ...
  fs.mkdirSync(path.join(root, '00_Documents', '00_Harness'), { recursive: true })

  fs.writeFileSync(
    path.join(root, 'package.json'),
    JSON.stringify({ name: 'fx', scripts: spec.packageScripts ?? { typecheck: 'tsc', test: 'vitest' } }, null, 2),
  )
  fs.writeFileSync(path.join(root, '00_Documents', '00_Harness', 'CORE.md'), spec.coreText)
  fs.writeFileSync(
    path.join(root, '00_Documents', '00_Harness', 'core-manifest.json'),
