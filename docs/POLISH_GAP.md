...
- **🏁 priority #1(AgentCodeGUI 활용) 사실상 완료** — 진입대문·Settings·단축키(Shift+Tab)·슬래시커맨드·스트리밍·탐색기갱신·GUI디테일. 잔여=P9(SubAgent 전이)·P7 광의 영속(draft/recentFiles, 저가치) 정도.
- **다음 = priority ② 디자인 테마**(주관적 — 사용자 방향 필요. 무인 전면 재디자인 위험 → 사용자 복귀 시 방향 확인 권장). 그 후 ③ 배포(M5).
