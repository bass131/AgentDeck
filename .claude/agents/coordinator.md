...
조건(`_routing.md`): `src/shared/**` 변경 / `AgentBackend`·`AgentEvent` 변경 / preload 노출 변경 / 위험 깃발 / ≥10줄+등급≥보통. 입력(`range`/`files`/`diff_summary`/`grade`/`flags`) 준비.
...
### "새 IPC 기능 추가" (복잡 표준)
1. `shared-ipc` — 채널명 + 요청/응답 타입 정의(`src/shared`) + preload 노출.
