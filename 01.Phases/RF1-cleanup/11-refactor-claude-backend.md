...
`02.Source/main/01_agents/ClaudeCodeBackend.ts`(2472줄, 최대 파일)를 책임 축으로 분리한다:
...
- [ ] 트랙 A 완료 권장
- [x] **Phase 07 — src/main 번호접두 이동** ✅완료(트랙B): `agents`→`01_agents` 번호접두 + 루트 재구성으로 현 위치 `02.Source/main/01_agents/`. 사전조건 충족 — 바로 내부 분해 가능.
...
> agent-backend (02.Source/main/01_agents/** R/W, Opus 권장) → reviewer 무조건 (backend-contract).
