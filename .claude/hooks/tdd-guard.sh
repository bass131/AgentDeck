...
case "$FP_N" in
  ...
  *02.Source/shared/*) exit 0 ;;                                    # 순수 타입/계약
  *.d.ts|*.config.ts|*.config.js|*/index.ts|*/preload/index.ts) exit 0 ;;
  *SampleData.ts|*sampleData.ts) exit 0 ;;                          # 목업/샘플 데이터(로직 없는 상수) — TDD 면제
  *02.Source/*.ts|*02.Source/*.tsx) : ;;                            # 대상
  *) exit 0 ;;                                                      # src 밖은 통과
esac

# 대응 테스트 추정: 파일명 기반으로 99.Others/tests/ 아래 *.test.* 존재 여부.
BASE="$(basename "$FP_N")"
STEM="${BASE%.*}"
PROJ="${CLAUDE_PROJECT_DIR:-.}"

if ls "$PROJ"/99.Others/tests/**/"$STEM".test.* "$PROJ"/99.Others/tests/"$STEM".test.* 2>/dev/null | grep -q . \
   || grep -rqs --include="*.test.*" "$STEM" "$PROJ/99.Others/tests" 2>/dev/null; then
  exit 0   # 대응 테스트 있음
fi
