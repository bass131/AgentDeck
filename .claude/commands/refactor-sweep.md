---
description: attended 자동 리팩토링 스윕 — SOLID/거대파일/중복 진단 + 안전 범위 자동 수정 + typecheck/test/lint 회귀 게이트 통과분만 전용 브랜치 atomic commit (push/PR 없음). 사용자가 세션을 감독할 때 호출.
...
> ⚠️ **이 슬래시는 사용자가 세션을 감독하는 동안 코드를 *수정하고 commit*한다** (`/review`는 읽기 전용). 자동 commit의 안전은 **로컬 commit까지만 + 회귀 게이트 + 전용 브랜치 + 선별 revert**에 달려 있다. 예약·백그라운드 무인 배치는 금지하며 push/PR은 *언제나* 사람 명시 GO(헌법 비가역 게이트).

---

### 핵심 안전 철학 (맨 위에 박는다)
- 자동 실행은 **로컬 commit까지만**. push/`gh pr create`/merge/배포는 *절대* 자동 X (G4).
...
- **신뢰경계 + ADR-003 엔진 추상화는 영원히 자동 적용 제외** (G7). renderer 시각/CSS는 *육안 검증 불가*라 제안만 (G3).
...
> - **✅ 자동 적용 (저위험)** — 거동 불변·가역·국소.
> - **🔶 자동 적용 (고위험)** — 구조 변경. 회귀 게이트 **+ Step 4 reviewer 재검증 필수** + 리포트 강조.
...
**판정 순서(위치 우선)**: (1) 신뢰경계/ADR-003 위치 → ⛔ (2) renderer 시각 → 📋 (3) 구조변경 → 🔶 자동 적용+재검증 (4) 저위험 → ✅ (5) reviewer 🟡 *진짜 모호* → 📋(다음 라운드).
...
```
─────────────────────────────────────────
🧹 attended 자동 리팩토링 스윕 완료 — YYYY-MM-DD
─────────────────────────────────────────
브랜치: refactor/auto-YYYYMMDD (출발: <원 브랜치>)
...
- **self-assessment bias = 고위험 cross-check** — reviewer가 *자기 슬래시로 자기 진단*을 후하게 볼 수 있음. **🔶 고위험 commit 모드 *첫 회차*는 2차 reviewer(또는 상향 모델) cross-check 1회 권장** — 단독 진단을 자동 commit 유일 게이트로 삼지 말 것.
...
| **v1** | ✅ 저위험만 자동 commit(🔶는 여전히 제안) | ✅ 저위험 |
