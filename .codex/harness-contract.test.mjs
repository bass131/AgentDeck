...
import { canaryRelative, canaryShouldRemove } from './harness-doctor.mjs'
...
test('doctor canary 처분 규칙: 이번 실행이 만든 파일만 삭제하고 사용자 파일은 보존한다 (Sol P2)', () => {
  // 존재하지 않던 곳에 쓰기 성공 → 우리가 만든 것 → 삭제 대상.
  assert.equal(canaryShouldRemove(false, true), true)
  // 이미 있던 파일(사용자 파일) → 절대 삭제하지 않는다.
  assert.equal(canaryShouldRemove(true, true), false)
  // 쓰기가 차단돼 아무것도 안 생긴 경우 → 삭제할 것 없음.
  assert.equal(canaryShouldRemove(false, false), false)
  assert.equal(canaryShouldRemove(true, false), false)
})

test('doctor canary 경로는 실행별 고유 토큰을 담아 동시 실행 충돌을 없앤다 (Sol P2)', () => {
