...
| **(b) 육안/취향** | 시각·UX·미감 — 자동 검증 불가 | **사용자 트랙**(병행) — 무인 commit X, 제안/스테이징까지 | renderer 시각·CSS·레이아웃(UI.md 안티슬롭, refactor-sweep 📋 제안만) |
| **(c) 비가역/판단** | 되돌리기 어렵거나 결정 성격 | **사람 게이트(ask)** — 무인 절대 X | push/PR/merge/배포·`package`·신뢰경계 구멍·ADR/헌법(`irreversible`/`trust-boundary`, refactor-sweep ⛔ 영구 제외) |
