...
/**
 * 표시 트리오 스냅샷 — loops/goal 배너가 읽는 필드.
 *
 * BL1 P03(stale-watchdog): autonomyActive·lastActivityAt 2필드 추가 — goal 배너 가시성의
 * 실제 게이트(autonomyActive, LR4 P05)가 이 레지스트리에 없으면 bgRuns cap(sessions.ts)/
 * panelManagerStates cap(panelSession.ts) 축출 이후 디스크·레지스트리 복원 경로에서 goal
 * 배너 자체가 사라져(gate 소실) stale 판정도 무의미해진다. 두 필드 모두 optional —
 * 기존 호출부(activeLoops/loopsStoppedNotice/pendingCommand만 채우는 리터럴)가 컴파일·런타임
 * 그대로 통과해야 하기 때문(회귀 0, sync()가 미전달 시 false/null로 기본값 채움).
 */
export interface LoopDisplaySnapshot {
  ...
  /** LR4 P05 실상태 게이트 — 미전달 시 sync()가 false로 채운다(기존 호출부 하위호환). */
...
export function isEmptyLoopDisplaySnapshot(v: LoopDisplaySnapshot): boolean {
  // BL1 P03: autonomyActive도 "표시할 무언가"에 포함 — goal이 done→continuation 사이
