...
## 2. 항상 사람 시선 (졸업 불가) `[문서 규범]`

⚠️ **처리량 문서 안에 있다고 "비용 축"으로 읽지 마라.** 이 절과 §3의 자기평가 편향 실측 전례는 **졸업의 상한**을 정하는 안전 장치이고, 어떤 훅도 이 목록을 강제하지 않는다. "속도를 위해 리뷰를 줄이자"는 정리는 정확히 여기부터 깎게 되는데, 여기가 마지막 방어선이다.
