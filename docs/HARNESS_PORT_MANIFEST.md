...
| 클래스 | 의미 | 행동 |
|---|---|---|
| **A — 그대로** | 프로세스 골격. 도메인 무관. | ClaudeDev 파일 가져와 솔로·폴더명만 적응. (밀고 새로 설치) |
...
| SubAgent 풀 표 | A골격/B내용 | 표 구조=A, 도메인 배정=B (AgentDeck 이미 번역) |
...
| `pr-and-merge-gate.md` | A(적응) — CODEOWNERS·admin 세부=솔로 적응 |
...
| `server` `client` `shared` `unity-bridge` | **B** → main-process·renderer·shared-ipc·agent-backend (AgentDeck 이미 번역) |
