...
| `--gold-glow-1` | `0.24` | `0.20` | 형광 pulse 저점 알파(신설, 2026-07-03 조정) — 숫자 값(색 아님), `oklch(from var(--gold) l c h / var(--gold-glow-1))`로 소비 |
| `--gold-glow-2` | `0.55` | `0.42` | 형광 pulse 고점 알파(신설, 2026-07-03 조정) — 라이트가 다크보다 높음(밝은 페이퍼 배경 위에서 씻겨 보이지 않도록 보정) |
